...
function option(name, status, description, source = 'plugin-doc', typeOrProvidesDefaults = 'string', providesDefaults = false) {
  const type = typeof typeOrProvidesDefaults === 'string' ? typeOrProvidesDefaults : 'string'
  return {
    name,
    status,
    description,
    source,
    type,
    providesDefaults: typeof typeOrProvidesDefaults === 'boolean' ? typeOrProvidesDefaults : providesDefaults,
  }
}
...
export const restorePluginHints = Object.freeze({
  barri: {
    ...
    aliases: ['barri'],
    options: [
      option('files', 'required', 'Comma-separated target devices or files to restore the BARRI image to; mutually exclusive with directory and copy.', 'plugin-doc', 'path'),
      option('directory', 'optional', 'Restore disks into generated files below this directory; mutually exclusive with files and copy.', 'plugin-doc', 'path'),
      option('copy', 'optional', 'Copy the BARRI image to this file for manual recovery with barri-cli; mutually exclusive with files and directory.', 'plugin-doc', 'path'),
  ...
  hyperV: {
    ...
    aliases: ['hyper-v'],
    options: [
      option('config_file', 'known', 'Path to the Hyper-V plugin configuration file.', 'plugin-example', true),
    ],
  },
  incus: {
    displayName: 'Incus',
    manualUrl: manualUrl('TasksAndConcepts/Plugins.html#incusplugin'),
    optionSeparator: ':',
    aliases: ['incus', 'bareos-fd-incus'],
    note: 'Options used at backup time are passed back on restore; restore_* options override the backup-time values.',
    options: [
      option('instance', 'required', 'Instance name to back up; can be overridden on restore with restore_instance.', 'plugin-doc'),
      option('project', 'optional', 'Incus project containing the instance.', 'plugin-doc'),
      option('remote', 'optional', 'Incus remote containing the instance.', 'plugin-doc'),
      option('allow_disk_resize', 'optional', 'Reserved option that currently must stay unset or no.', 'plugin-doc', 'boolean'),
      option('chunk_size', 'optional', 'Virtual-machine disk chunk size, e.g. 64MiB.', 'plugin-doc'),
      option('chunk_id_length', 'optional', 'Number of digits used for chunk identifiers.', 'plugin-doc', 'integer'),
      option('compression', 'optional', 'Tarball compression algorithm, e.g. none, lzma, gzip, bzip2, or zstd where supported.', 'plugin-doc'),
      option('path_prefix', 'optional', 'Bareos storage path prefix for backup files.', 'plugin-doc'),
      option('queue_depth', 'optional', 'Maximum number of chunks buffered in RAM during backup.', 'plugin-doc', 'integer'),
      option('max_file_size', 'optional', 'Maximum file size allowed in the backup buffer.', 'plugin-doc'),
      option('backup_poll_timeout', 'optional', 'Delay after which a quiet Incus export is considered stalled.', 'plugin-doc'),
      option('hash', 'optional', 'Hash algorithm used to detect changes in each chunk.', 'plugin-doc'),
      option('hijacked_stat_fields', 'optional', 'Comma-separated stat fields used to store chunk digests.', 'plugin-doc'),
      option('restore_buffer_depth', 'optional', 'Maximum number of restore chunks buffered in RAM.', 'plugin-doc', 'integer'),
      option('restore_config_override', 'optional', 'Repeatable <key>=<value> configuration override applied on restore.', 'plugin-doc'),
      option('restore_device_override', 'optional', 'Repeatable <device>,<key>=<value> device override applied on restore.', 'plugin-doc'),
      option('restore_instance', 'optional', 'Target instance name for restore; defaults to instance.', 'plugin-doc'),
      option('restore_path', 'optional', 'Write the restore tarball to this filesystem path instead of importing it.', 'plugin-doc', 'path'),
      option('restore_project', 'optional', 'Target project for restore; defaults to project.', 'plugin-doc'),
      option('restore_remote', 'optional', 'Target remote for restore; defaults to remote.', 'plugin-doc'),
      option('restore_storage', 'optional', 'Target storage pool for restore.', 'plugin-doc'),
      option('temp_dir', 'optional', 'Temporary directory used for restore chunks.', 'plugin-doc', 'path'),
    ],
  },
  ldap: {
    displayName: 'LDAP',
  ...
  libcloud: {
    ...
    options: [
      ...
      option('config_file', 'optional', 'Path to the libcloud configuration file.', 'plugin-doc', true),
  ...
  mariabackup: {
    ...
    options: [
      option('mycnf', 'optional', 'Path to the my.cnf file containing connection credentials.', 'plugin-doc', true),
  ...
  mariadbDump: {
    ...
    options: [
      ...
      option('defaultsfile', 'known', 'Path to a MariaDB defaults file for client utilities.', 'plugin-readme', true),
  ...
  mysqlDump: {
    ...
    options: [
      ...
      option('drop_and_recreate', 'known', 'Disable drop/create statements when set to false.', 'plugin-readme'),
      option('defaultsfile', 'known', 'Path to a defaults file for mysql and mysqldump.', 'plugin-readme', true),
  ...
  proxmox: {
    ...
    options: [
      option('guestid', 'optional', 'Guest ID to create or restore; required when restoring directly into Proxmox.', 'plugin-doc', 'integer'),
  ...
  qumulo: {
    ...
    note: 'This is a third-party plugin. Either share_name or path must be set; config_file can provide the same values.',
    options: [
      option('host', 'required', 'Qumulo cluster host name or address.', 'plugin-source'),
      option('port', 'required', 'Qumulo API TCP port.', 'plugin-source', 'integer'),
      option('user', 'required', 'Qumulo API user name.', 'plugin-source'),
      option('password', 'required', 'Qumulo API password.', 'plugin-source'),
      option('mount_point', 'required', 'Local mount point used by the plugin.', 'plugin-source', 'path'),
      option('snapshot_name', 'required', 'Qumulo snapshot name used for the backup.', 'plugin-source'),
      option('share_name', 'optional', 'Qumulo share to back up; either share_name or path must be set.', 'plugin-source'),
      option('path', 'optional', 'Qumulo filesystem path to back up; either share_name or path must be set.', 'plugin-source', 'path'),
      option('config_file', 'optional', 'Path to a Qumulo plugin configuration file.', 'plugin-source', 'path', true),
      option('api_limit', 'optional', 'Maximum number of entries requested per Qumulo API call.', 'plugin-source', 'integer'),
      option('max_workers', 'optional', 'Maximum number of worker processes.', 'plugin-source', 'integer'),
      option('exclusions', 'optional', 'Path to a file containing glob patterns to exclude.', 'plugin-source', 'path'),
      option('exclusions_report', 'optional', 'Whether to report excluded objects.', 'plugin-source', 'boolean'),
      option('injections', 'optional', 'Path to a file containing objects to inject into the backup.', 'plugin-source', 'path'),
      option('timeout', 'optional', 'Qumulo API request timeout in seconds.', 'plugin-source', 'integer'),
      option('perf_interval', 'optional', 'Performance logging interval; 0 disables reporting.', 'plugin-source', 'integer'),
      option('io_type', 'optional', 'Where file I/O is performed: core or plugin.', 'plugin-source'),
    ],
  },
  python: {
    ...
    options: [
      ...
      option('defaults_file', 'known', 'Configuration file whose values act as defaults.', 'plugin-doc', true),
      option('overrides_file', 'known', 'Configuration file whose values override Fileset options.', 'plugin-doc', true),
    ],
  },
  tasksMariadb: {
    ...
    options: [
      ...
      option('defaultsfile', 'known', 'Defaults file used by mariadb client utilities.', 'plugin-readme', true),
  ...
  tasksMysql: {
    ...
    options: [
      ...
      option('defaultsfile', 'known', 'Defaults file used by MySQL client utilities.', 'plugin-readme', true),
      option('databases', 'known', 'Comma-separated list of databases to include.', 'plugin-readme'),
      option('exclude', 'known', 'Comma-separated list of databases to exclude.', 'plugin-readme'),
    ],
  },
  ...
  vmware: {
    ...
    options: [
      ...
      option('restore_powerstate', 'optional', 'Power state after restore: on, off, or previous.', 'plugin-doc'),
      ...
      option('config_file', 'optional', 'Path to a configuration file with shared VMware plugin options.', 'plugin-doc', true),
