...
static inline PyRestorePacket* NativeToPyRestorePacket(restore_pkt* rp)
{
  ...
  auto* orig_fname = rp->original_file_name
                         ? PyUnicode_FromString(rp->original_file_name)
                         : OwnedNone();
  auto* orig_lname = rp->original_link_name
  ...
  if (pRestorePacket && olname && attrEx && orig_fname && orig_lname && ofname
      && statp) {
    ...
    pRestorePacket->original_file_name = orig_fname;
    pRestorePacket->original_link_name = orig_lname;
    ...
    free(where);
    free(RegexWhere);
    Py_XDECREF(orig_fname);
    Py_XDECREF(orig_lname);
    Py_XDECREF(ofname);
    return nullptr;
  }
}
  ...
  switch (var) {
    ...
    case bVarAccurateOptions: {
      ...
      if (bareos_core_functions->getBareosValue(plugin_ctx, var, &value)
        ...
        if (((value & static_cast<uint64_t>(accurate_inode))
             && !add_option(accurate_inode))
            || ((value & static_cast<uint64_t>(accurate_permissions))
                && !add_option(accurate_permissions))
            || ((value & static_cast<uint64_t>(accurate_nlink))
                && !add_option(accurate_nlink))
            || ((value & static_cast<uint64_t>(accurate_uid))
                && !add_option(accurate_uid))
            || ((value & static_cast<uint64_t>(accurate_gid))
                && !add_option(accurate_gid))
            || ((value & static_cast<uint64_t>(accurate_size))
                && !add_option(accurate_size))
            || ((value & static_cast<uint64_t>(accurate_atime))
                && !add_option(accurate_atime))
            || ((value & static_cast<uint64_t>(accurate_mtime))
                && !add_option(accurate_mtime))
            || ((value & static_cast<uint64_t>(accurate_ctime))
                && !add_option(accurate_ctime))
            || ((value & static_cast<uint64_t>(accurate_size_decrease))
                && !add_option(accurate_size_decrease))
            || ((value & static_cast<uint64_t>(accurate_always))
                && !add_option(accurate_always))
            || ((value & static_cast<uint64_t>(accurate_md5))
                && !add_option(accurate_md5))
            || ((value & static_cast<uint64_t>(accurate_sha1))
                && !add_option(accurate_sha1))) {
...
{
  auto* original_file_name = PyGetStringValue(self->original_file_name);
  if (!original_file_name) { return nullptr; }
  ...
  auto* original_link_name = PyGetStringValue(self->original_link_name);
  if (!original_link_name) { return nullptr; }
  ...
  Mmsg(buf,
       ...
       self->RegexWhere, self->replace, self->create_status, original_file_name,
       original_link_name);
                                ...
                                PyObject* kwds)
{
  ...
  bool ok = PyArg_ParseTupleAndKeywords(
      ...
      "|iiiiiIOOOOssiiiOO",
#endif
      kwlist, &self->stream, &self->data_stream, &self->type, &self->file_index,
      &self->LinkFI, &self->uid, &self->statp, &self->attrEx, &self->ofname,
      &self->olname, &where, &RegexWhere, &self->replace, &self->create_status,
      &self->filedes, &self->original_file_name, &self->original_link_name);
...
static int PyIoPacket_init(PyIoPacket* self, PyObject* args, PyObject* kwds)
{
  ...
  bool ok = PyArg_ParseTupleAndKeywords(
      args, kwds, "|HiiiOUiiiiLpL", kwlist, &self->func, &self->count,
