                                ...
                                e_list_type type)
{
  ...
  if (count) {
    ...
    if (mdbr->VolumeName[0] != 0) {
      FillQuery<SQL_QUERY::list_volumes_by_name_count_1>(query, esc->c_str());
                                   ...
                                   e_list_type type)
{
  ...
  if (VolumeName && VolumeName[0] != 0) {
    const auto escaped_volume_name = EscapeString(jcr, VolumeName);
    if (!escaped_volume_name) { return; }
    PoolMem filter(PM_MESSAGE);
    filter.bsprintf("AND Media.VolumeName='%s' ", escaped_volume_name->c_str());
