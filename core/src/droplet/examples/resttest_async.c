...
void cb_delete_object(void* handle)
{
...
void delete_object()
{
  ...
  atask = (dpl_async_task_t*)dpl_delete_async_prepare(
      ctx,              // the context
      NULL,             // no bucket
      new_path,         // the key
      NULL,             // no option
...
void cb_head_object(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
  ...
  fprintf(stderr, "file %s: size=%ld mtime=%lu\n", atask->u.head.resource,
          atask->u.head.sysmd.size, atask->u.head.sysmd.mtime);
...
void cb_list_bucket(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
  int i;
  ...
  for (i = 0; i < atask->u.list_bucket.objects->n_items; i++) {
    dpl_object_t* obj
        = (dpl_object_t*)dpl_vec_get(atask->u.list_bucket.objects, i);
    ...
    sub_atask = (dpl_async_task_t*)dpl_head_async_prepare(
        ctx,
        NULL,  // no bucket
        obj->path,
        NULL,             // option
  ...
  for (i = 0; i < atask->u.list_bucket.common_prefixes->n_items; i++) {
    dpl_common_prefix_t* dir = (dpl_common_prefix_t*)dpl_vec_get(
        atask->u.list_bucket.common_prefixes, i);
...
again:
  ...
  if (n_ok != atask->u.list_bucket.objects->n_items) goto again;
...
void list_bucket()
{
  ...
  atask = (dpl_async_task_t*)dpl_list_bucket_async_prepare(ctx, NULL, folder,
                                                           "/");
...
void cb_get_metadata(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
...
void get_metadata()
{
  ...
  atask = (dpl_async_task_t*)dpl_head_async_prepare(
      ctx,              // the context
      NULL,             // no bucket,
      new_path,         // the key
      NULL,             // no option
...
void cb_update_metadata(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
...
void cb_get_object(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
  int i;
  dpl_dict_var_t* metadatum = NULL;
...
void cb_rename_object(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
...
void rename_object(char* resource_path)
{
  dpl_async_task_t* atask = NULL;
  dpl_buf_t* buf = NULL;
  dpl_status_t ret;
...
void cb_create_object(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
