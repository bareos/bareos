 ...
 * 5) appending data to an existing object, or automatically creating it if it
 * doesn't already exist and adding the data 6) adding metadata to added objects
 * (for named/unnamed objects) 7) getting object contents (and metadata) by ID
 ...
 * 11) copying ID object with changed metadata (is the content duplicated in the
 * storage?) 12) copying named object with changed metadata (is the content
...
void cb_copy_named_object_with_new_md(void* handle)
{
...
void copy_named_object_with_new_md()
{
  ...
  atask = (dpl_async_task_t*)dpl_copy_async_prepare(
      ctx,
      NULL,                     // no src bucket
...
void cb_copy_nameless_object_with_new_md(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
...
void copy_nameless_object_with_new_md()
{
  ...
  /*
   * note: With Dewpoint, it would be possible to copy nameless object into
   * another nameless object. Does it make sense ? for now we copy it into a
   * named object
   */
  atask = (dpl_async_task_t*)dpl_copy_id_async_prepare(
      ctx,
      NULL,                     // no src bucket
...
void cb_get_object_meta(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
  int i;
...
void cb_get_named_object_partially(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
  int i;
  dpl_dict_var_t* metadatum = NULL;
...
void get_named_object_partially()
{
  dpl_async_task_t* atask = NULL;
  dpl_buf_t* buf = NULL;
  dpl_status_t ret;
...
void cb_get_named_object(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
  int i;
  dpl_dict_var_t* metadatum = NULL;
  ...
  printf("data:\n");
  write(1, dpl_buf_ptr(atask->u.get.buf),
        MIN(dpl_buf_size(atask->u.get.buf), 10));
  printf("...\nmetadata:\n");
  dpl_dict_print(atask->u.get.metadata, stdout, 0);
...
void cb_get_nameless_object(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
  int i;
  dpl_dict_var_t* metadatum = NULL;
...
void get_nameless_object()
{
  ...
  atask = (dpl_async_task_t*)dpl_get_id_async_prepare(
      ctx,            // the context
      NULL,           // no bucket
      id1,            // the key
      NULL,           // no opion
...
void cb_update_metadata_nameless_object(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
...
void update_metadata_nameless_object()
{
  dpl_async_task_t* atask = NULL;
  dpl_status_t ret;
  dpl_option_t option;
  ...
  atask = (dpl_async_task_t*)dpl_put_id_async_prepare(
      ctx,
      NULL,           // no bucket
      id1,            // the id
      &option,        // option
...
void cb_update_metadata_named_object(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
...
void update_metadata_named_object()
{
  dpl_async_task_t* atask = NULL;
  dpl_status_t ret;
  dpl_option_t option;
  ...
  atask = (dpl_async_task_t*)dpl_put_async_prepare(
      ctx,
      NULL,           // no bucket
      file3_path,     // the id
      &option,        // option
...
void cb_append_to_nonexisting_named_object(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
...
void append_to_nonexisting_named_object()
{
  dpl_async_task_t* atask = NULL;
  dpl_buf_t* buf = NULL;
  dpl_status_t ret;
  ...
  atask = (dpl_async_task_t*)dpl_post_async_prepare(
      ctx,
      NULL,           // no bucket
      file3_path,     // the id
      NULL,           // option
...
void cb_append_to_nonexisting_named_object_precond(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
...
void append_to_nonexisting_named_object_precond()
{
  dpl_async_task_t* atask = NULL;
  dpl_buf_t* buf = NULL;
  dpl_status_t ret;
...
void cb_add_existing_named_object(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
...
void add_existing_named_object()
{
  dpl_async_task_t* atask = NULL;
  dpl_buf_t* buf = NULL;
  dpl_status_t ret;
...
void cb_add_existing_named_object_no_precond(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
...
void add_existing_named_object_no_precond()
{
  dpl_async_task_t* atask = NULL;
  dpl_buf_t* buf = NULL;
  dpl_status_t ret;
...
void cb_add_nonexisting_named_object(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
...
void add_nonexisting_named_object()
{
  dpl_async_task_t* atask = NULL;
  dpl_buf_t* buf = NULL;
  dpl_status_t ret;
...
void cb_add_nameless_object(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
...
void add_nameless_object()
{
  ...
  atask = (dpl_async_task_t*)dpl_post_id_async_prepare(
      ctx,            // the context
      NULL,           // no bucket
      NULL,           // no id
      NULL,           // no option
...
void cb_make_folder(void* handle)
{
  dpl_async_task_t* atask = (dpl_async_task_t*)handle;
