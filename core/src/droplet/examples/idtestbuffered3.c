/*
 * simple buffered example which use ID capable REST services (emulate blocks
 * with UKS)
 */
...
char* my_dpl_uks_bn2hex(const BIGNUM* id)
{
  char* id_str = NULL;
...
int main(int argc, char** argv)
{
  ...
  while (remain_size > 0) {
    ...
    ret = dpl_scal_gc_index_serialize(
        id,
...
free_all:
  ...
  if (NULL != gc_id) free(gc_id);

  if (NULL != main_id) free(main_id);

  if (NULL != id) BN_free(id);
  ...
  if (NULL != dbuf) dpl_dbuf_free(dbuf);
