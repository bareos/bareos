...
static int toyserver_wait(pid_t pid)
{
  int r;
  int status;
  ...
  for (;;) {
    ...
    if (r < 0) {
      ...
      if (errno == ESRCH) return -1;  /* it's gone */
      if (errno == ECHILD) return -1; /* it's gone */
    ...
    if (WIFEXITED(status)) {
      if (!WEXITSTATUS(status)) return 0; /* normal successful exit */
      if (verbose)
 ...
 */
static void toyserver_shutdown(pid_t pid)
{
  kill(pid, SIGTERM);
  toyserver_wait(pid);
}

int toyserver_start(const char* name, struct server_state** statep)
{
  int fd = -1;
  int r;
  struct stat sb;
  ...
  snprintf(statefile, sizeof(statefile), "%s.state", name ? name : "toyserver");

  if (verbose) fprintf(stderr, "Starting toyserver\n");
  ...
  p = mmap(NULL, sizeof(struct server_state), PROT_READ | PROT_WRITE,
           MAP_SHARED, fd, 0);
  if (p == MAP_FAILED) {
    perror("mmap");
    goto error;
  ...
  *statep = (struct server_state*)p;
  if (verbose) fprintf(stderr, "Started toyserver pid %d\n", (*statep)->pid);
  return 0;

error:
  if (fd >= 0) close(fd);
  if (pid > 1) toyserver_shutdown(pid);
  return -1;
}

void toyserver_stop(struct server_state* state)
{
  if (state) {
    if (verbose)
...
const char* toyserver_addrlist(struct server_state* state)
{
  static char buf[128];
