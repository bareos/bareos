...
static void kv_add(struct kv** listp, const char* key, const char* value)
{
  struct kv *p, **tailp;
...
static void kv_set(struct kv** listp, const char* key, const char* value)
{
  struct kv *p, **prevp;
...
static void kv_free(struct kv** listp)
{
  while (*listp) {
    struct kv* p = *listp;
    *listp = (*listp)->next;
...
static const char* kv_ifind(struct kv** listp, const char* key)
{
  struct kv** pp;

  for (pp = listp; *pp; pp = &(*pp)->next) {
    if (!strcasecmp(key, (*pp)->key)) return (*pp)->value;
  }
  return NULL;
}
 ...
 * `state' pointer.
 */
static int setup_state(const char* statefile)
{
  int fd;
  int n;
  void* p;
  ...
  if (verbose) fprintf(stderr, "Statefile locked, truncating and zeroing\n");
  ...
  if (verbose) fprintf(stderr, "Mapping statefile\n");
...
static int create_server_socket(int* portp)
{
  ...
  if (verbose) fprintf(stderr, "Creating rendezvous socket\n");
  ...
  if (*portp) sin.sin_port = htons(*portp);
...
static int server_accept(void)
{
  ...
  if (verbose) fprintf(stderr, "Server waiting for connection\n");
  ...
  if (verbose) fprintf(stderr, "Server accepted connection\n");
 ...
 */
static void server_unaccept(void)
{
  struct connection* conn = &state->connection;
 ...
 */
static int server_getline(char* buf, int maxlen)
{
  if (!fgets(buf, maxlen, state->connection.in)) return -1;
  if (verbose) fprintf(stderr, "C: %s", buf);
  return 0;
}
...
static int strmap_to_value(const struct strmap* map, const char* str)
{
  if (str == NULL) return -1;
  for (; map->name; map++) {
    if (!strcasecmp(map->name, str)) return map->value;
  }
  return -1;
}

static const char* strmap_to_name(const struct strmap* map, int value)
{
  for (; map->name; map++) {
    if (value == map->value) return map->name;
  }
  return NULL;
}

static const struct strmap version_strs[]
    = {{"HTTP/1.0", V_1_0}, {"HTTP/1.1", V_1_1}, {NULL, 0}};
static const struct strmap method_strs[]
    = {{"OPTIONS", M_OPTIONS}, {"GET", M_GET},         {"HEAD", M_HEAD},
       {"POST", M_POST},       {"PUT", M_PUT},         {"DELETE", M_DELETE},
       {"TRACE", M_TRACE},     {"CONNECT", M_CONNECT}, {NULL, 0}};
...
static void server_wait_for_request(void)
{
  int c = getc(state->connection.in);
  if (c != EOF) ungetc(c, state->connection.in);
}

static void server_setup_request(void)
{
...
static int server_read_request_start_line(void)
{
  struct request* req = &state->request;
  int r;
  char* p;
  ...
  p = strtok(buf, " ");
  if (p == NULL) return 400; /* Bad Request */
  if (verbose) fprintf(stderr, "method=\"%s\"\n", p);
  ...
  if (verbose) fprintf(stderr, "uri=\"%s\"\n", req->uri);
  ...
  /* Check the method */
  if (req->method < 0) return 400; /* Bad Request */

  if (verbose) fprintf(stderr, "Good request\n");
  return 200;
}

static int server_set_request_authorization(char* str)
{
  struct request* req = &state->request;
  BIO* bio;
  char* p;
  ...
  p = strtok(str, sep);
  if (strcasecmp(p, "basic")) return 401;
  ...
  BIO_free_all(bio);
  if (len <= 0 || len >= sizeof(buf)) return 401;
  buf[len] = '\0';
...
static int request_is_persistent(struct request* req)
{
  const char* p;

  if (req->version == V_1_0) return 0; /* HTTP/1.0 */

  /* HTTP/1.1 */

  p = kv_ifind(&req->headers, "Connection");
  if (p && !strcasecmp(p, "close")) return 0;
...
static int server_read_request_headers(void)
{
  struct request* req = &state->request;
  ...
  while (server_getline(buf, sizeof(buf)) == 0) {
    ...
    if (buf[0] == ' ' || buf[0] == '\t') {
      if (verbose) fprintf(stderr, "Found continuation header, failing\n");
    ...
    *p++ = '\0';
    ...
    if (!strcasecmp(buf, "Host")) {
      ...
      if (authorization_seen++) return 400; /* Bad Request */
...
static int server_read_request_body(void)
{
  struct request* req = &state->request;
  int remain;
  char* p;
  ...
  if (req->body_len >= sizeof(req->body)) {
    if (verbose) fprintf(stderr, "Request is just too large sorry\n");
    return -1;
  }
...
/* Returns 0 on success, an HTTP status or -1 on error */
static int server_handle_request(void)
{
  struct request* req = &state->request;
  ...
  if (state->config.require_basic_auth) {
    ...
    if (!req->has_authorization) return 401;             /* Unauthorized */
    if (strcmp(req->username, TOY_USERNAME)) return 403; /* Forbidden */
    if (strcmp(req->password, TOY_PASSWORD)) return 403; /* Forbidden */
  }
  ...
  if (!strncmp(req->uri, "/proxy/chord/", 13)) {
    ...
    switch (req->method) {
      case M_PUT:
        if (verbose) fprintf(stderr, "Fake sproxyd PUT\n");
        kv_set(&objects, id, req->body);
        return 0;
      case M_GET:
        if (verbose) fprintf(stderr, "Fake sproxyd GET\n");
        p = kv_ifind(&objects, id);
        if (NULL == p) return 404; /* Not Found */
  ...
  if (verbose) fprintf(stderr, "Faking a trivial reply for now\n");
  return 0;
}

static void server_setup_reply(void)
{
  struct reply* rep = &state->reply;
...
static int server_send_reply(void)
{
  struct reply* rep = &state->reply;
  int r;
  int remain;
  ...
  if (verbose) fprintf(stderr, "Sending reply\n");
  ...
  server_printf("%s %03d %s\r\n",
                ...
                rep->status, ss ? ss : "Unknown status");
  ...
  /* Reply body */
  if (rep->body_len) {
    if (verbose) fprintf(stderr, "S: ...%d bytes...\n", rep->body_len);
 ...
 * connection.
 */
static void server_request_loop(void)
{
  int r;
  ...
  do {
    ...
    if (!r) return; /* EOF */
    if (r != 200) goto error_and_disconnect;

    r = server_read_request_headers();
    if (r) goto error_and_disconnect;
 ...
 */
static int server_setup(int foreground_flag, const char* statefile)
{
  int r;
  pid_t pid;
  int sync_pipe[2];
  ...
  if (verbose) fprintf(stderr, "Initialising server!\n");
  ...
  if (!foreground_flag) {
    ...
    if (pid) {
      ...
      char dummy;
      close(sync_pipe[PIPE_WRITE]);
      if (verbose) fprintf(stderr, "Parent waiting for child to initialise\n");
    ...
    close(sync_pipe[PIPE_READ]);
    if (verbose) fprintf(stderr, "Child pid is %d\n", (int)getpid());
  }

  if (setup_state(statefile) < 0) goto error;
  ...
  if (!foreground_flag) {
    ...
    char dummy = 'X';
    if (verbose) fprintf(stderr, "Child signalling parent\n");
...
error:
  ...
  if (!foreground_flag) close(sync_pipe[PIPE_WRITE]);
  return -1;
}
...
int main(int argc, char** argv)
{
  ...
  if (verbose) fprintf(stderr, "Toyserver starting\n");

  if (server_setup(foreground_flag, statefile) < 0) return 1;
