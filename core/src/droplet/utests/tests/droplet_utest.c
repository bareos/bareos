...
#define TESTCASE(str, maskval, expvers, forvers)  \
  ...
  TESTCASE("lazy: http_compat: raw: append_metadata: consistent:",
           DPL_OPTION_LAZY | DPL_OPTION_HTTP_COMPAT | DPL_OPTION_RAW
               | DPL_OPTION_APPEND_METADATA | DPL_OPTION_CONSISTENT,
           "", "");
...
#define TESTCASE(str, err)               \
  ...
  TESTCASE(
      "if-none-match:"
      "ecf3245105b18821eda700c279ba9ae39af72c169574ea47e3c37037744fb444710f4f31"
      "d655894305f380b0a8c1aa83",
      DPL_EINVAL);
  /* an invalid time string */
  TESTCASE("if-modified-since:356-Flubuary-20789", DPL_EINVAL);
  /* too many conditions */
  TESTCASE(
      "if-match:a if-match:b if-match:c if-match:d if-match:e "
...
#define TESTCASE(str, ...)                                 \
  ...
  TESTCASE("if-modified-since:15-Oct-2010()03:19:52()+1100",
           {.type = DPL_CONDITION_IF_MODIFIED_SINCE, .time = 1287073192L});
  TESTCASE("if-unmodified-since:5-Oct-2010()03:19:52()+1100",
           {.type = DPL_CONDITION_IF_UNMODIFIED_SINCE, .time = 1286209192L});
  /* multiple conditions */
