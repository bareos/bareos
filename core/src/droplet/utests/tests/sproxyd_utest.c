...
static void setup(void)
{
  ...
  dpl_assert_int_eq(
      DPL_SUCCESS, dpl_dict_add(profile, "host", toyserver_addrlist(state), 0));
  dpl_assert_int_eq(DPL_SUCCESS,
                    dpl_dict_add(profile, "backend", "sproxyd", 0));
  dpl_assert_int_eq(DPL_SUCCESS,
                    dpl_dict_add(profile, "base_path", "/proxy/chord", 0));
  dpl_assert_int_eq(DPL_SUCCESS,
                    dpl_dict_add(profile, "droplet_dir", "/never/seen", 0));
  dpl_assert_int_eq(DPL_SUCCESS,
...
START_TEST(get_set_test)
{
  ...
  s = dpl_gen_random_key(ctx, DPL_STORAGE_CLASS_STANDARD, /*custom*/ NULL, id,
                         sizeof(id));
  dpl_assert_int_eq(DPL_SUCCESS, s);

  s = dpl_put_id(ctx, "foobucket", id,
                 /*options*/ NULL, DPL_FTYPE_REG,
                 /*condition*/ NULL,
                 /*range*/ NULL,
                 /*metadata*/ NULL,
                 /*sysmd*/ NULL, data, sizeof(data) - 1);
  dpl_assert_int_eq(DPL_SUCCESS, s);
  ...
  //   /*condition*/NULL, /*range*/NULL, char **data_bufp, unsigned int
  //   *data_lenp, dpl_dict_t **metadatap, dpl_sysmd_t *sysmdp);
  //   dpl_assert_int_eq(DPL_SUCCESS, s);
...
static dpl_status_t send_authenticated_request(const char* username,
                                               const char* password,
                                               struct request* reqp)
{
  dpl_status_t s;
  ...
  if (username)
    dpl_assert_int_eq(DPL_SUCCESS,
                      dpl_dict_add(prof2, "access_key", username, 0));
  if (password)
    dpl_assert_int_eq(DPL_SUCCESS,
                      dpl_dict_add(prof2, "secret_key", password, 0));
  ...
  s = dpl_put_id(ctx, "foobucket", id,
                 ...
                 /*metadata*/ NULL,
                 /*sysmd*/ NULL, data, sizeof(data) - 1);
  if (reqp) *reqp = state->request; /* sample before disconnection */
...
#define TESTCASE(u, p, ehasa, eu, ep, es)            \
  ...
  TESTCASE(NULL, NULL,
           /*has_authorization*/ 0, "", "", DPL_FAILURE);
  ...
  TESTCASE(TOY_USERNAME, TOY_PASSWORD,
           /*has_authorization*/ 1, TOY_USERNAME, TOY_PASSWORD, DPL_SUCCESS);
#undef TESTCASE
}
END_TEST
