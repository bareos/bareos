...
static int check_keys(const char* k, int kl, void* arg)
{
...
static char* new_pseudorandom_bytes(int n)
{
  unsigned char* x;
  int i;
  unsigned long bits;
...
START_TEST(ntinydb_test)
{
  ...
  for (i = 0; i < all_keys.nelem; i++) { all_keys.found[i] = false; }
