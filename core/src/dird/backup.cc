...
static bool GetBaseJobids(JobControlRecord* jcr, db_list_ctx* jobids)
{
  ...
  if (!jcr->impl_->res.job->base) {
    return false; /* no base job, stop accurate */
  }

  jr.StartTime = jcr->impl_->jr.StartTime;

  foreach_alist (job, jcr->impl_->res.job->base) {
...
static int AccurateListHandler(void* ctx, int num_fields, char** row)
{
  ...
  if (jcr->impl_->use_accurate_chksum && num_fields == 9 &&
...
static bool IsChecksumNeededByFileset(JobControlRecord* jcr)
{
  ...
  if (!jcr->impl_->res.job || !jcr->impl_->res.job->fileset) { return false; }
...
bool SendAccurateCurrentFiles(JobControlRecord* jcr)
{
  ...
  if (jcr->is_JobLevel(L_FULL)) {
     ...
     */
    jcr->db->AccurateGetJobids(jcr, &jcr->impl_->jr, &jobids);
  ...
  if (jcr->HasBase) {
    ...
    if (!jcr->db->GetBaseFileList(jcr, jcr->impl_->use_accurate_chksum,
    ...
    jcr->db_batch->GetFileList(
        jcr, jobids.list, jcr->impl_->use_accurate_chksum, false /* no delta */,
        AccurateListHandler, (void*)jcr);
  }
...
bool DoNativeBackup(JobControlRecord* jcr)
{
   ...
   */
  jcr->passive_client = jcr->impl_->res.client->passive;
  ...
  } else if (jcr->impl_->res.client->max_bandwidth > 0) {
    jcr->max_bandwidth = jcr->impl_->res.client->max_bandwidth;
  }
...
int WaitForJobTermination(JobControlRecord* jcr, int timeout)
{
  ...
  if (fd) {
    ...
    while ((n = BgetDirmsg(fd)) >= 0) {
      if (!fd_ok &&
          sscanf(fd->msg, EndJob, &jcr->impl_->FDJobStatus, &JobFiles,
                 &ReadBytes, &JobBytes, &JobErrors, &VSS, &Encrypt) == 7) {
        fd_ok = true;
        jcr->setJobStatus(jcr->impl_->FDJobStatus);
    ...
    if (IsBnetError(fd)) {
      ...
      while (i++ < 10 && jcr->impl_->res.job->RescheduleIncompleteJobs &&
             jcr->IsCanceled()) {
        Bmicrosleep(3, 0);
      }
    }
  ...
  Dmsg5(100, "cancel=%d fd_ok=%d FDJS=%d JS=%d SDJS=%d\n", jcr->IsCanceled(),
        fd_ok, jcr->impl_->FDJobStatus, jcr->JobStatus,
        jcr->impl_->SDJobStatus);
  if (jcr->IsCanceled() ||
      (!jcr->impl_->res.job->RescheduleIncompleteJobs && !fd_ok)) {
    Dmsg4(100, "fd_ok=%d FDJS=%d JS=%d SDJS=%d\n", fd_ok,
          jcr->impl_->FDJobStatus, jcr->JobStatus, jcr->impl_->SDJobStatus);
  ...
  if (!fd_ok || IsBnetError(fd)) { /* if fd not set, that use !fd_ok */
    jcr->impl_->FDJobStatus = JS_ErrorTerminated;
  }
  if (jcr->JobStatus != JS_Terminated) { return jcr->JobStatus; }
  if (jcr->impl_->FDJobStatus != JS_Terminated) {
    return jcr->impl_->FDJobStatus;
  }
  return jcr->impl_->SDJobStatus;
}
...
void UpdateBootstrapFile(JobControlRecord* jcr)
{
  ...
  if (jcr->IsTerminatedOk() && jcr->impl_->jr.JobBytes &&
      jcr->impl_->res.job->WriteBootstrap) {
    FILE* fd;
    int VolCount;
    int got_pipe = 0;
    Bpipe* bpipe = NULL;
    ...
    fname = edit_job_codes(jcr, fname, jcr->impl_->res.job->WriteBootstrap, "");
    if (*fname == '|') {
      got_pipe = 1;
    ...
    if (fd) {
      ...
      if (VolCount == 0) {
        Jmsg(jcr, M_ERROR, 0,
             ...
             jcr->db->strerror());
        if (jcr->impl_->SDJobFiles != 0) {
          jcr->setJobStatus(JS_ErrorTerminated);
        }
      }
      /* Start output with when and who wrote it */
      bstrftimes(edt, sizeof(edt), time(NULL));
      fprintf(fd, "# %s - %s - %s%s\n", edt, jcr->impl_->jr.Job,
              JobLevelToString(jcr->getJobLevel()), jcr->impl_->since);
...
void GenerateBackupSummary(JobControlRecord *jcr, ClientDbRecord *cr, int msg_type, const char *TermMsg)
{
   ...
   if (RunTime <= 0) {
      ...
      kbps = ((double)jcr->impl_->jr.JobBytes) / (1000.0 * (double)RunTime);
   }

   if (!jcr->db->GetJobVolumeNames(jcr, jcr->impl_->jr.JobId, jcr->VolumeName)) {
      /*
   ...
   case PT_NDMP_BAREOS:
      ...
      Mmsg(statistics, _(
           ...
           edit_uint64_with_commas(jcr->impl_->jr.JobFiles, ec1),
           edit_uint64_with_commas(jcr->impl_->SDJobFiles, ec2),
           edit_uint64_with_commas(jcr->impl_->jr.JobBytes, ec3),
           edit_uint64_with_suffix(jcr->impl_->jr.JobBytes, ec4),
           edit_uint64_with_commas(jcr->impl_->SDJobBytes, ec5),
           edit_uint64_with_suffix(jcr->impl_->SDJobBytes, ec6));
      break;
   case PT_NDMP_NATIVE:
      Mmsg(level_info, _(
           "  Backup Level:           %s%s\n"),
   ...
   if (jcr->impl_->HasQuota) {
      if (jcr->impl_->res.client->GraceTime != 0) {
         bstrftimes(gdt, sizeof(gdt), jcr->impl_->res.client->GraceTime +
                                      jcr->impl_->res.client->SoftQuotaGracePeriod);
      } else {
      ...
      Mmsg(quota_info, _(
           ...
           "  Hard Quota:             %s (%sB)\n"
           "  Grace Expiry Date:      %s\n"),
           edit_uint64_with_commas(jcr->impl_->jr.JobSumTotalBytes+jcr->impl_->SDJobBytes, ec1),
           edit_uint64_with_suffix(jcr->impl_->jr.JobSumTotalBytes+jcr->impl_->SDJobBytes, ec2),
           edit_uint64_with_commas(jcr->impl_->res.client->QuotaLimit, ec3),
           edit_uint64_with_suffix(jcr->impl_->res.client->QuotaLimit, ec4),
           edit_uint64_with_commas(jcr->impl_->res.client->SoftQuota, ec5),
           edit_uint64_with_suffix(jcr->impl_->res.client->SoftQuota, ec6),
           edit_uint64_with_commas(jcr->impl_->res.client->HardQuota, ec7),
           edit_uint64_with_suffix(jcr->impl_->res.client->HardQuota, ec8),
           gdt);
   }
      ...
      if (jcr->is_JobLevel(L_VIRTUAL_FULL)) {
         ...
         if (jcr->HasBase) {
            Mmsg(client_options, _(
                 ...
                 jcr->impl_->VSS ? _("yes") : _("no"),
                 jcr->impl_->Encrypt ? _("yes") : _("no"),
                 jcr->accurate ? _("yes") : _("no"));
         } else {
            Mmsg(client_options, _(
                 "  Software Compression:   %s%s\n"
         ...
         if (!bstrcmp(jcr->impl_->FDSecureEraseCmd, "*None*")) {
            Mmsg(temp, "  FD  Secure Erase Cmd:   %s\n", jcr->impl_->FDSecureEraseCmd);
            PmStrcat(secure_erase_status, temp.c_str());
         }
         if (!bstrcmp(jcr->impl_->SDSecureEraseCmd, "*None*")) {
...
// Bmicrosleep(15, 0);                /* for debugging SIGHUP */
   ...
   Jmsg(jcr, msg_type, 0, _("%s %s %s (%s):\n"
        ...
        jcr->impl_->jr.Job,
        level_info.c_str(),
        jcr->impl_->res.client->resource_name_, cr->Uname,
        jcr->impl_->res.fileset->resource_name_, jcr->impl_->FSCreateTime,
        jcr->impl_->res.pool->resource_name_, jcr->impl_->res.pool_source,
        jcr->impl_->res.catalog->resource_name_, jcr->impl_->res.catalog_source,
        jcr->impl_->res.write_storage->resource_name_, jcr->impl_->res.wstore_source,
        schedt,
        sdt,
        edt,
