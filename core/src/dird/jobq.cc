...
int JobqAdd(jobq_t* jq, JobControlRecord* jcr)
{
  ...
  if (!jcr->impl_->term_wait_inited) {
     ...
     */
    if ((status = pthread_cond_init(&jcr->impl_->term_wait, NULL)) != 0) {
      BErrNo be;
...
extern "C" void* jobq_server(void* arg)
{
  ...
  for (;;) {
    ...
    while (!jq->ready_jobs->empty() && !jq->quit) {
       ...
       * queue.
       */
      if (jcr->impl_->acquired_resource_locks) {
        DecReadStore(jcr);
        DecWriteStore(jcr);
        DecClientConcurrency(jcr);
        DecJobConcurrency(jcr);
        jcr->impl_->acquired_resource_locks = false;
      }
    ...
    if (!jq->waiting_jobs->empty() && !jq->quit) {
      ...
      if (re) {
        ...
        for (; re;) {
          Dmsg2(
              ...
              re->jcr->impl_->res.job->allow_mixed_priority ? "mix" : "no mix");
          if (!re->jcr->impl_->res.job->allow_mixed_priority) {
      ...
      for (; je;) {
        ...
        Dmsg4(2300, "Examining Job=%d JobPri=%d want Pri=%d (%s)\n", jcr->JobId,
              ...
              jcr->impl_->res.job->allow_mixed_priority ? "mix" : "no mix");
        ...
        if (!(jcr->JobPriority == Priority ||
              (jcr->JobPriority < Priority &&
               jcr->impl_->res.job->allow_mixed_priority &&
               running_allow_mix))) {
...
static bool RescheduleJob(JobControlRecord* jcr, jobq_t* jq, jobq_item_t* je)
{
   ...
   */
  if (jcr->impl_->res.job->RescheduleTimes == 0 ||
      jcr->impl_->reschedule_count < jcr->impl_->res.job->RescheduleTimes) {
    resched =
        /*
         * Check for incomplete jobs
         */
        (jcr->impl_->res.job->RescheduleIncompleteJobs && jcr->IsIncomplete() &&
         jcr->is_JobType(JT_BACKUP) && !jcr->is_JobLevel(L_BASE)) ||
        /*
         * Check for failed jobs
         */
        (jcr->impl_->res.job->RescheduleOnError && !jcr->IsTerminatedOk() &&
  ...
  if (resched) {
     ...
     */
    now = time(NULL);
    jcr->impl_->reschedule_count++;
    jcr->sched_time = now + jcr->impl_->res.job->RescheduleInterval;
    ...
    Dmsg4(2300, "Rescheduled Job %s to re-run in %d seconds.(now=%u,then=%u)\n",
          jcr->Job, (int)jcr->impl_->res.job->RescheduleInterval, now,
          jcr->sched_time);
    Jmsg(jcr, M_INFO, 0,
         _("Rescheduled Job %s at %s to re-run in %d seconds (%s).\n"),
         jcr->Job, dt, (int)jcr->impl_->res.job->RescheduleInterval, dt2);
    ...
    if (jcr->JobBytes == 0) {
      ...
      jcr->setJobStatus(JS_WaitStartTime);
      njcr = NewDirectorJcr();
      SetJcrDefaults(njcr, jcr->impl_->res.job);
      njcr->impl_->reschedule_count = jcr->impl_->reschedule_count;
      njcr->sched_time = jcr->sched_time;
      njcr->initial_sched_time = jcr->initial_sched_time;

      njcr->setJobLevel(jcr->getJobLevel());
      njcr->impl_->res.pool = jcr->impl_->res.pool;
      njcr->impl_->res.run_pool_override = jcr->impl_->res.run_pool_override;
      njcr->impl_->res.full_pool = jcr->impl_->res.full_pool;
      njcr->impl_->res.run_full_pool_override =
          jcr->impl_->res.run_full_pool_override;
      njcr->impl_->res.inc_pool = jcr->impl_->res.inc_pool;
      njcr->impl_->res.run_inc_pool_override =
          jcr->impl_->res.run_inc_pool_override;
      njcr->impl_->res.diff_pool = jcr->impl_->res.diff_pool;
      njcr->impl_->res.run_diff_pool_override =
          jcr->impl_->res.run_diff_pool_override;
      njcr->impl_->res.next_pool = jcr->impl_->res.next_pool;
      njcr->impl_->res.run_next_pool_override =
          jcr->impl_->res.run_next_pool_override;
      njcr->JobStatus = -1;
      njcr->setJobStatus(jcr->JobStatus);
      if (jcr->impl_->res.read_storage) {
        CopyRstorage(njcr, jcr->impl_->res.read_storage_list,
                     _("previous Job"));
      } else {
        FreeRstorage(njcr);
      }
      if (jcr->impl_->res.write_storage) {
        CopyWstorage(njcr, jcr->impl_->res.write_storage_list,
                     _("previous Job"));
      } else {
        FreeWstorage(njcr);
      }
      njcr->impl_->res.messages = jcr->impl_->res.messages;
      njcr->impl_->spool_data = jcr->impl_->spool_data;
...
static bool AcquireResources(JobControlRecord* jcr)
{
  ...
  switch (jcr->getJobType()) {
    ...
    case JT_CONSOLIDATE:
       ...
       */
      jcr->impl_->IgnoreClientConcurrency = true;
      ...
      if (jcr->impl_->MigrateJobId == 0) {
        ...
        Dmsg1(200,
              ...
              jcr->Job);
        jcr->impl_->IgnoreStorageConcurrency = true;
      }
      break;
    default:
      break;
  ...
  jcr->impl_->acquired_resource_locks = true;
...
static bool IncClientConcurrency(JobControlRecord* jcr)
{
  if (!jcr->impl_->res.client || jcr->impl_->IgnoreClientConcurrency) {
    return true;
  }

  P(mutex);
  if (jcr->impl_->res.client->rcs->NumConcurrentJobs <
      jcr->impl_->res.client->MaxConcurrentJobs) {
    jcr->impl_->res.client->rcs->NumConcurrentJobs++;
...
static void DecClientConcurrency(JobControlRecord* jcr)
{
  if (jcr->impl_->IgnoreClientConcurrency) { return; }
...
{
  P(mutex);
  if (jcr->impl_->res.job->rjs->NumConcurrentJobs <
      jcr->impl_->res.job->MaxConcurrentJobs) {
    jcr->impl_->res.job->rjs->NumConcurrentJobs++;
...
{
  if (jcr->impl_->IgnoreStorageConcurrency) { return true; }

  P(mutex);
  if (jcr->impl_->res.read_storage->runtime_storage_status->NumConcurrentJobs <
      jcr->impl_->res.read_storage->MaxConcurrentJobs) {
    jcr->impl_->res.read_storage->runtime_storage_status
        ->NumConcurrentReadJobs++;
    jcr->impl_->res.read_storage->runtime_storage_status->NumConcurrentJobs++;
...
{
  if (jcr->impl_->res.read_storage && !jcr->impl_->IgnoreStorageConcurrency) {
    P(mutex);
    jcr->impl_->res.read_storage->runtime_storage_status
        ->NumConcurrentReadJobs--;
    jcr->impl_->res.read_storage->runtime_storage_status->NumConcurrentJobs--;
...
static bool IncWriteStore(JobControlRecord* jcr)
{
  ...
  if (jcr->impl_->res.write_storage->runtime_storage_status->NumConcurrentJobs <
      jcr->impl_->res.write_storage->MaxConcurrentJobs) {
    jcr->impl_->res.write_storage->runtime_storage_status->NumConcurrentJobs++;
    Dmsg2(50, "Inc Wstore=%s wncj=%d\n",
          jcr->impl_->res.write_storage->resource_name_,
          jcr->impl_->res.write_storage->runtime_storage_status
              ->NumConcurrentJobs);
    V(mutex);
...
static void DecWriteStore(JobControlRecord* jcr)
{
  if (jcr->impl_->res.write_storage && !jcr->impl_->IgnoreStorageConcurrency) {
    P(mutex);
    jcr->impl_->res.write_storage->runtime_storage_status->NumConcurrentJobs--;
    ...
    if (jcr->impl_->res.write_storage->runtime_storage_status
            ->NumConcurrentJobs < 0) {
