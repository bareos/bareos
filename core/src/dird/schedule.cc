...
// (constructor)
Schedule::Schedule()
   : day_mask(std::make_tuple(All<WeekOfYear>(), All<DayOfWeek>())), times({ { TimeOfDay(0, 0) } }) {}
Schedule::Schedule(Mask<MonthOfYear> month_of_year, Mask<WeekOfMonth> week_of_month, Mask<DayOfWeek> day_of_week, List<TimeOfDay> time)
   : day_mask(std::make_tuple(month_of_year, week_of_month, day_of_week)), times(time) {}
Schedule::Schedule(Mask<MonthOfYear> month_of_year, Mask<DayOfWeek> day_of_week, List<TimeOfDay> time)
   : day_mask(std::make_tuple(month_of_year, All<WeekOfMonth>(), day_of_week)), times(time) {}
Schedule::Schedule(Mask<MonthOfYear> month_of_year, Mask<DayOfMonth> day_of_month, List<TimeOfDay> time)
   : day_mask(std::make_tuple(month_of_year, day_of_month)), times(time) {}
Schedule::Schedule(Mask<WeekOfYear> week_of_year, Mask<DayOfWeek> day_of_week, List<TimeOfDay> time)
   : day_mask(std::make_tuple(week_of_year, day_of_week)), times(time) {}
Schedule::Schedule(Mask<WeekOfMonth> week_of_month, Mask<DayOfWeek> day_of_week, List<TimeOfDay> time)
   : day_mask(std::make_tuple(All<MonthOfYear>(), week_of_month, day_of_week)), times(time) {}
Schedule::Schedule(Mask<DayOfMonth> day_of_month, List<TimeOfDay> time)
   : day_mask(std::make_tuple(All<MonthOfYear>(), day_of_month)), times(time) {}
Schedule::Schedule(Mask<DayOfWeek> day_of_week, List<TimeOfDay> time)
   : day_mask(std::make_tuple(All<WeekOfYear>(), day_of_week)), times(time) {}
Schedule::Schedule(List<TimeOfDay> time)
...
std::vector<time_t> Schedule::GetMatchingTimes(time_t from, time_t to) const {
   ...
   if (TriggersOnDay(date_time)) {
      for (const auto& time : times.items) {
