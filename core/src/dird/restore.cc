...
static void BuildRestoreCommand(JobControlRecord* jcr, PoolMem& ret)
{
   ...
   */
  if (jcr->impl_->replace != 0) {
    replace = jcr->impl_->replace;
  } else if (jcr->impl_->res.job->replace != 0) {
    replace = jcr->impl_->res.job->replace;
  } else {
  ...
  if (jcr->RegexWhere) {
    ...
    cmd = restorecmdR;
  } else if (jcr->impl_->res.job->RegexWhere) {
    where = jcr->impl_->res.job->RegexWhere; /* no override take from job */
    cmd = restorecmdR;
  } else if (jcr->where) {
    where = jcr->where; /* override */
    cmd = restorecmd;
  } else if (jcr->impl_->res.job->RestoreWhere) {
    where = jcr->impl_->res.job->RestoreWhere; /* no override take from job */
    cmd = restorecmd;
  ...
  jcr->prefix_links = jcr->impl_->res.job->PrefixLinks;
...
static inline bool DoNativeRestoreBootstrap(JobControlRecord* jcr)
{
  ...
  while (!feof(info.bs)) {
    ...
    if (first_time) {
      ...
      jcr->impl_->keep_sd_auth_key = true; /* don't clear the sd_auth_key now */
    ...
    if (first_time) {
       ...
       */
      if (jcr->impl_->FDVersion >= FD_VERSION_52) {
    ...
    if (jcr->impl_->FDVersion < FD_VERSION_2) { /* Old FD */
  ...
  if (fd && jcr->impl_->FDVersion >= FD_VERSION_2) { fd->fsend("endrestore"); }
...
void NativeRestoreCleanup(JobControlRecord* jcr, int TermCode)
{
  ...
  switch (TermCode) {
    case JS_Terminated:
      if (jcr->impl_->ExpectedFiles > jcr->impl_->jr.JobFiles) {
  ...
  switch (jcr->getJobProtocol()) {
    ...
    case PT_NDMP_NATIVE:
      Jmsg(jcr, msg_type, 0,
           ...
           jcr->impl_->res.client->resource_name_, sdt, edt,
           edit_utime(RunTime, elapsed, sizeof(elapsed)),
           edit_uint64_with_commas((uint64_t)jcr->impl_->ExpectedFiles, ec1),
           edit_uint64_with_commas((uint64_t)jcr->impl_->jr.JobFiles, ec2),
           edit_uint64_with_commas(jcr->impl_->jr.JobBytes, ec3), (float)kbps,
