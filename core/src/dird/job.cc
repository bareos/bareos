...
bool SetupJob(JobControlRecord* jcr, bool suppress_output)
{
   ...
   */
  if (!suppress_output) {
    InitMsg(jcr, jcr->impl_->res.messages, job_code_callback_director);
  } else {
   ...
   */
  if ((errstat = pthread_cond_init(&jcr->impl_->term_wait, NULL)) != 0) {
    BErrNo be;
  ...
  jcr->impl_->term_wait_inited = true;

  /*
   * Initialize nextrun ready condition variable
   */
  if ((errstat = pthread_cond_init(&jcr->impl_->nextrun_ready, NULL)) != 0) {
    BErrNo be;
    Jmsg1(jcr, M_FATAL, 0,
  ...
  jcr->impl_->nextrun_ready_inited = true;

  CreateUniqueJobName(jcr, jcr->impl_->res.job->resource_name_);
  ...
  if (!jcr->impl_->res.pool_source) {
    jcr->impl_->res.pool_source = GetPoolMemory(PM_MESSAGE);
    PmStrcpy(jcr->impl_->res.pool_source, _("unknown source"));
  }

  if (!jcr->impl_->res.npool_source) {
    jcr->impl_->res.npool_source = GetPoolMemory(PM_MESSAGE);
    PmStrcpy(jcr->impl_->res.npool_source, _("unknown source"));
  }

  if (jcr->JobReads()) {
    if (!jcr->impl_->res.rpool_source) {
      jcr->impl_->res.rpool_source = GetPoolMemory(PM_MESSAGE);
      PmStrcpy(jcr->impl_->res.rpool_source, _("unknown source"));
    }
  }
  ...
  if (!jcr->db->CreateJobRecord(jcr, &jcr->impl_->jr)) {
    Jmsg(jcr, M_FATAL, 0, "%s", jcr->db->strerror());
    goto bail_out;
  }

  jcr->JobId = jcr->impl_->jr.JobId;
  Dmsg4(100, "Created job record JobId=%d Name=%s Type=%c Level=%c\n",
        jcr->JobId, jcr->Job, jcr->impl_->jr.JobType, jcr->impl_->jr.JobLevel);
  ...
  if (jcr->JobReads() && !jcr->impl_->res.read_storage_list) {
    if (jcr->impl_->res.job->storage) {
      CopyRwstorage(jcr, jcr->impl_->res.job->storage, _("Job resource"));
    } else {
      CopyRwstorage(jcr, jcr->impl_->res.job->pool->storage,
                    _("Pool resource"));
    }
  }
    ...
    case JT_RESTORE:
      switch (jcr->getJobProtocol()) {
        ...
        default:
           ...
           */
          if (jcr->impl_->restore_tree_root) {
...
{
  return IsConnectingToClientAllowed(jcr->impl_->res.client);
}
...
{
  return IsConnectFromClientAllowed(jcr->impl_->res.client);
}
...
bool UseWaitingClient(JobControlRecord* jcr, int timeout)
{
  ...
  if (!IsConnectFromClientAllowed(jcr)) {
    ...
    connection =
        connections->remove(jcr->impl_->res.client->resource_name_, timeout);
    if (connection) {
      jcr->file_bsock = connection->bsock();
      jcr->impl_->FDVersion = connection->protocol_version();
...
static void* job_thread(void* arg)
{
  ...
  if (jcr->impl_->res.job->MaxStartDelay != 0 &&
      jcr->impl_->res.job->MaxStartDelay <
   ...
   */
  if (jcr->impl_->res.job->RunScripts == NULL) {
    Dmsg0(200, "Warning, job->RunScripts is empty\n");
    jcr->impl_->res.job->RunScripts = new alist(10, not_owned_by_alist);
  }
   ...
   */
  RunScripts(jcr, jcr->impl_->res.job->RunScripts, "BeforeJob");
  ...
  RunScripts(jcr, jcr->impl_->res.job->RunScripts, "AfterJob");
...
{
  jcr->lock();
  if (!jcr->impl_->sd_msg_thread_done && jcr->impl_->SD_msg_chan_started &&
      !pthread_equal(jcr->impl_->SD_msg_chan, pthread_self())) {
    Dmsg1(800, "Send kill to SD msg chan jid=%d\n", jcr->JobId);
    pthread_kill(jcr->impl_->SD_msg_chan, sig);
  }
  jcr->unlock();
}
...
bool CancelJob(UaContext* ua, JobControlRecord* jcr)
{
  ...
  switch (old_status) {
    ...
    default:
       ...
       */
      if (jcr->impl_->mig_jcr && jcr->impl_->mig_jcr->store_bsock) {
        if (!CancelStorageDaemonJob(ua, jcr->impl_->mig_jcr)) { return false; }
      }
...
static void JobMonitorWatchdog(watchdog_t* self)
{
  ...
  foreach_jcr (jcr) {
    ...
    if (jcr->JobId == 0 || JobCanceled(jcr) || jcr->impl_->no_maxtime) {
...
{
  bool cancel = false;
  JobResource* job = jcr->impl_->res.job;
  utime_t current = 0;
...
static bool JobCheckMaxrunschedtime(JobControlRecord* jcr)
{
  if (jcr->impl_->MaxRunSchedTime == 0 || JobCanceled(jcr)) { return false; }
  if ((watchdog_time - jcr->initial_sched_time) < jcr->impl_->MaxRunSchedTime) {
    Dmsg3(200, "Job %p (%s) with MaxRunSchedTime %d not expired\n", jcr,
          jcr->Job, jcr->impl_->MaxRunSchedTime);
    return false;
  }
...
bool AllowDuplicateJob(JobControlRecord* jcr)
{
   ...
   */
  if (job->AllowDuplicateJobs || jcr->impl_->IgnoreDuplicateJobChecking) {
    return true;
  }
  ...
  foreach_jcr (djcr) {
     ...
     */
    if (djcr->impl_->IgnoreDuplicateJobChecking) { continue; }

    if (bstrcmp(job->resource_name_, djcr->impl_->res.job->resource_name_)) {
...
bool GetLevelSinceTime(JobControlRecord* jcr)
{
   ...
   */
  if (jcr->impl_->cloned && jcr->stime && jcr->stime[0]) {
    bstrncpy(jcr->impl_->since, _(", since="), sizeof(jcr->impl_->since));
    bstrncat(jcr->impl_->since, jcr->stime, sizeof(jcr->impl_->since));
    return pool_updated;
  }
  ...
  switch (JobLevel) {
    ...
    case L_INCREMENTAL:
      ...
      jcr->impl_->jr.JobId = 0; /* flag to return since time */

      /*
       * This is probably redundant, but some of the code below
       * uses jcr->stime, so don't remove unless you are sure.
       */
      if (!jcr->db->FindJobStartTime(jcr, &jcr->impl_->jr, jcr->stime,
                                     jcr->impl_->PrevJob)) {
        do_full = true;
      }

      have_full = jcr->db->FindLastJobStartTime(jcr, &jcr->impl_->jr, stime,
                                                prev_job, L_FULL);
      if (have_full) {
      ...
      if (have_full && JobLevel == L_INCREMENTAL &&
          jcr->impl_->res.job->MaxDiffInterval > 0) {
        /*
         * Lookup last diff job
         */
        if (jcr->db->FindLastJobStartTime(jcr, &jcr->impl_->jr, stime, prev_job,
                                          L_DIFFERENTIAL)) {
        ...
        do_diff =
            ((now - last_diff_time) >= jcr->impl_->res.job->MaxDiffInterval);
        Dmsg2(50, "do_diff=%d diffInter=%lld\n", do_diff,
              jcr->impl_->res.job->MaxDiffInterval);
      }

      /*
       * Note, do_full takes precedence over do_vfull and do_diff
       */
      if (have_full && jcr->impl_->res.job->MaxFullInterval > 0) {
        do_full =
            ((now - last_full_time) >= jcr->impl_->res.job->MaxFullInterval);
      } else if (have_full && jcr->impl_->res.job->MaxVFullInterval > 0) {
        do_vfull =
            ((now - last_full_time) >= jcr->impl_->res.job->MaxVFullInterval);
      }
      ...
      if (do_full) {
        ...
        Jmsg(jcr, M_INFO, 0,
             _("No prior or suitable Full backup found in catalog. Doing FULL "
               "backup.\n"));
        Bsnprintf(jcr->impl_->since, sizeof(jcr->impl_->since),
                  _(" (upgraded from %s)"), JobLevelToString(JobLevel));
        jcr->setJobLevel(jcr->impl_->jr.JobLevel = L_FULL);
        pool_updated = true;
      } else if (do_vfull) {
        /*
        ...
        jcr->setJobLevel(jcr->impl_->jr.JobLevel = L_VIRTUAL_FULL);
        pool_updated = true;
        ...
        Bsnprintf(jcr->impl_->since, sizeof(jcr->impl_->since),
                  _(" (upgraded from %s)"), JobLevelToString(JobLevel));
        jcr->setJobLevel(jcr->impl_->jr.JobLevel = L_DIFFERENTIAL);
        pool_updated = true;
      } else {
        if (jcr->impl_->res.job->rerun_failed_levels) {
          if (jcr->db->FindFailedJobSince(jcr, &jcr->impl_->jr, jcr->stime,
                                          JobLevel)) {
            Jmsg(jcr, M_INFO, 0,
            ...
            Bsnprintf(jcr->impl_->since, sizeof(jcr->impl_->since),
                      _(" (upgraded from %s)"), JobLevelToString(JobLevel));
            jcr->setJobLevel(jcr->impl_->jr.JobLevel = JobLevel);
       ...
       */
      if (jcr->impl_->PrevJob[0]) {
        bstrncpy(jcr->impl_->previous_jr.Job, jcr->impl_->PrevJob,
                 sizeof(jcr->impl_->previous_jr.Job));
...
void ApplyPoolOverrides(JobControlRecord* jcr, bool force)
{
   ...
   */
  if (!force && jcr->impl_->IgnoreLevelPoolOverides) { return; }

  /*
   * If only a pool override and no level overrides are given in run entry
   * choose this pool
   */
  if (jcr->impl_->res.run_pool_override &&
      !jcr->impl_->res.run_full_pool_override &&
      !jcr->impl_->res.run_vfull_pool_override &&
      !jcr->impl_->res.run_inc_pool_override &&
      !jcr->impl_->res.run_diff_pool_override) {
    PmStrcpy(jcr->impl_->res.pool_source, _("Run Pool override"));
    Dmsg2(100, "Pool set to '%s' because of %s",
          jcr->impl_->res.pool->resource_name_, _("Run Pool override\n"));
  } else {
    /*
    ...
    switch (jcr->getJobLevel()) {
      case L_FULL:
        if (jcr->impl_->res.full_pool) {
          jcr->impl_->res.pool = jcr->impl_->res.full_pool;
          pool_override = true;
          if (jcr->impl_->res.run_full_pool_override) {
            PmStrcpy(jcr->impl_->res.pool_source, _("Run FullPool override"));
            Dmsg2(100, "Pool set to '%s' because of %s",
                  jcr->impl_->res.full_pool->resource_name_,
                  "Run FullPool override\n");
          } else {
            PmStrcpy(jcr->impl_->res.pool_source, _("Job FullPool override"));
            Dmsg2(100, "Pool set to '%s' because of %s",
                  jcr->impl_->res.full_pool->resource_name_,
        ...
        break;
      case L_VIRTUAL_FULL:
        if (jcr->impl_->res.vfull_pool) {
          jcr->impl_->res.pool = jcr->impl_->res.vfull_pool;
          pool_override = true;
          if (jcr->impl_->res.run_vfull_pool_override) {
            PmStrcpy(jcr->impl_->res.pool_source, _("Run VFullPool override"));
            Dmsg2(100, "Pool set to '%s' because of %s",
                  jcr->impl_->res.vfull_pool->resource_name_,
                  "Run VFullPool override\n");
          } else {
            PmStrcpy(jcr->impl_->res.pool_source, _("Job VFullPool override"));
            Dmsg2(100, "Pool set to '%s' because of %s",
                  jcr->impl_->res.vfull_pool->resource_name_,
        ...
        break;
      case L_INCREMENTAL:
        if (jcr->impl_->res.inc_pool) {
          jcr->impl_->res.pool = jcr->impl_->res.inc_pool;
          pool_override = true;
          if (jcr->impl_->res.run_inc_pool_override) {
            PmStrcpy(jcr->impl_->res.pool_source, _("Run IncPool override"));
            Dmsg2(100, "Pool set to '%s' because of %s",
                  jcr->impl_->res.inc_pool->resource_name_,
                  "Run IncPool override\n");
          } else {
            PmStrcpy(jcr->impl_->res.pool_source, _("Job IncPool override"));
            Dmsg2(100, "Pool set to '%s' because of %s",
                  jcr->impl_->res.inc_pool->resource_name_,
        ...
        break;
      case L_DIFFERENTIAL:
        if (jcr->impl_->res.diff_pool) {
          jcr->impl_->res.pool = jcr->impl_->res.diff_pool;
          pool_override = true;
          if (jcr->impl_->res.run_diff_pool_override) {
            PmStrcpy(jcr->impl_->res.pool_source, _("Run DiffPool override"));
            Dmsg2(100, "Pool set to '%s' because of %s",
                  jcr->impl_->res.diff_pool->resource_name_,
                  "Run DiffPool override\n");
          } else {
            PmStrcpy(jcr->impl_->res.pool_source, _("Job DiffPool override"));
            Dmsg2(100, "Pool set to '%s' because of %s",
                  jcr->impl_->res.diff_pool->resource_name_,
   ...
   */
  if (pool_override && jcr->impl_->res.pool->catalog) {
    jcr->impl_->res.catalog = jcr->impl_->res.pool->catalog;
    PmStrcpy(jcr->impl_->res.catalog_source, _("Pool resource"));
  }
}
...
bool GetOrCreateClientRecord(JobControlRecord* jcr)
{
  ...
  cr.AutoPrune = jcr->impl_->res.client->AutoPrune;
  cr.FileRetention = jcr->impl_->res.client->FileRetention;
  cr.JobRetention = jcr->impl_->res.client->JobRetention;
  if (!jcr->client_name) { jcr->client_name = GetPoolMemory(PM_NAME); }
  PmStrcpy(jcr->client_name, jcr->impl_->res.client->resource_name_);
   ...
   */
  if (jcr->impl_->res.client->HardQuota != 0 ||
      jcr->impl_->res.client->SoftQuota != 0) {
    if (!jcr->db->GetQuotaRecord(jcr, &cr)) {
      ...
      jcr->impl_->res.client->QuotaLimit = 0;
      jcr->impl_->res.client->GraceTime = 0;
    }
  }
  jcr->impl_->jr.ClientId = cr.ClientId;
  jcr->impl_->res.client->QuotaLimit = cr.QuotaLimit;
  jcr->impl_->res.client->GraceTime = cr.GraceTime;
  if (cr.Uname[0]) {
    if (!jcr->impl_->client_uname) {
      jcr->impl_->client_uname = GetPoolMemory(PM_NAME);
    }
    PmStrcpy(jcr->impl_->client_uname, cr.Uname);
  }
  Dmsg2(100, "Created Client %s record %d\n",
        jcr->impl_->res.client->resource_name_, jcr->impl_->jr.ClientId);
  return true;
}
...
bool GetOrCreateFilesetRecord(JobControlRecord* jcr)
{
   ...
   */
  bstrncpy(fsr.FileSet, jcr->impl_->res.fileset->resource_name_,
           sizeof(fsr.FileSet));
  if (jcr->impl_->res.fileset->have_MD5) {
    MD5_CTX md5c;
    unsigned char digest[MD5HashSize];
    memcpy(&md5c, &jcr->impl_->res.fileset->md5c, sizeof(md5c));
    ...
    bstrncpy(jcr->impl_->res.fileset->MD5, fsr.MD5,
             sizeof(jcr->impl_->res.fileset->MD5));
  } else {
    Jmsg(jcr, M_WARNING, 0, _("FileSet MD5 digest not found.\n"));
  }
  if (!jcr->impl_->res.fileset->ignore_fs_changes ||
      !jcr->db->GetFilesetRecord(jcr, &fsr)) {
    PoolMem FileSetText(PM_MESSAGE);

    jcr->impl_->res.fileset->PrintConfig(FileSetText, *my_config, false, false);
  ...
  jcr->impl_->jr.FileSetId = fsr.FileSetId;
  bstrncpy(jcr->impl_->FSCreateTime, fsr.cCreateTime,
           sizeof(jcr->impl_->FSCreateTime));

  Dmsg2(119, "Created FileSet %s record %u\n",
        jcr->impl_->res.fileset->resource_name_, jcr->impl_->jr.FileSetId);
...
{
  jcr->impl_->jr.SchedTime = jcr->sched_time;
  jcr->impl_->jr.StartTime = jcr->start_time;
  jcr->impl_->jr.EndTime = 0; /* perhaps rescheduled, clear it */
  jcr->impl_->jr.JobType = jcr->getJobType();
  jcr->impl_->jr.JobLevel = jcr->getJobLevel();
  jcr->impl_->jr.JobStatus = jcr->JobStatus;
  jcr->impl_->jr.JobId = jcr->JobId;
  jcr->impl_->jr.JobSumTotalBytes = 18446744073709551615LLU;
  bstrncpy(jcr->impl_->jr.Name, jcr->impl_->res.job->resource_name_,
           sizeof(jcr->impl_->jr.Name));
  bstrncpy(jcr->impl_->jr.Job, jcr->Job, sizeof(jcr->impl_->jr.Job));
}
...
{
  jcr->impl_->jr.EndTime = time(NULL);
  jcr->end_time = jcr->impl_->jr.EndTime;
  jcr->impl_->jr.JobId = jcr->JobId;
  jcr->impl_->jr.JobStatus = jcr->JobStatus;
  jcr->impl_->jr.JobFiles = jcr->JobFiles;
  jcr->impl_->jr.JobBytes = jcr->JobBytes;
  jcr->impl_->jr.ReadBytes = jcr->ReadBytes;
  jcr->impl_->jr.VolSessionId = jcr->VolSessionId;
  jcr->impl_->jr.VolSessionTime = jcr->VolSessionTime;
  jcr->impl_->jr.JobErrors = jcr->JobErrors;
  jcr->impl_->jr.HasBase = jcr->HasBase;
  if (!jcr->db->UpdateJobEndRecord(jcr, &jcr->impl_->jr)) {
...
void DirdFreeJcrPointers(JobControlRecord* jcr)
{
  ...
  BfreeAndNull(jcr->impl_->backup_format);
  BfreeAndNull(jcr->RestoreBootstrap);
  BfreeAndNull(jcr->ar);

  FreeAndNullPoolMemory(jcr->JobIds);
  FreeAndNullPoolMemory(jcr->impl_->client_uname);
  FreeAndNullPoolMemory(jcr->attr);
  FreeAndNullPoolMemory(jcr->impl_->fname);
}
...
void DirdFreeJcr(JobControlRecord* jcr)
{
  ...
  if (jcr->impl_->mig_jcr) {
    FreeJcr(jcr->impl_->mig_jcr);
    jcr->impl_->mig_jcr = NULL;
  }

  DirdFreeJcrPointers(jcr);

  if (jcr->impl_->term_wait_inited) {
    pthread_cond_destroy(&jcr->impl_->term_wait);
    jcr->impl_->term_wait_inited = false;
  }

  if (jcr->impl_->nextrun_ready_inited) {
    pthread_cond_destroy(&jcr->impl_->nextrun_ready);
    jcr->impl_->nextrun_ready_inited = false;
  }
  ...
  FreeAndNullPoolMemory(jcr->impl_->fname);
  FreeAndNullPoolMemory(jcr->impl_->res.pool_source);
  FreeAndNullPoolMemory(jcr->impl_->res.npool_source);
  FreeAndNullPoolMemory(jcr->impl_->res.rpool_source);
  FreeAndNullPoolMemory(jcr->impl_->res.wstore_source);
  FreeAndNullPoolMemory(jcr->impl_->res.rstore_source);
  FreeAndNullPoolMemory(jcr->impl_->res.catalog_source);
  FreeAndNullPoolMemory(jcr->impl_->FDSecureEraseCmd);
  FreeAndNullPoolMemory(jcr->impl_->SDSecureEraseCmd);
  FreeAndNullPoolMemory(jcr->impl_->vf_jobids);
...
void SetJcrDefaults(JobControlRecord* jcr, JobResource* job)
{
  ...
  if (!jcr->impl_->res.catalog_source) {
    jcr->impl_->res.catalog_source = GetPoolMemory(PM_MESSAGE);
    PmStrcpy(jcr->impl_->res.catalog_source, _("unknown source"));
  }
  ...
  PmStrcpy(jcr->impl_->res.pool_source, _("Job resource"));
  jcr->impl_->res.pool = job->pool;
  jcr->impl_->res.full_pool = job->full_pool;
  jcr->impl_->res.inc_pool = job->inc_pool;
  jcr->impl_->res.diff_pool = job->diff_pool;

  if (job->pool && job->pool->catalog) {
    jcr->impl_->res.catalog = job->pool->catalog;
    PmStrcpy(jcr->impl_->res.catalog_source, _("Pool resource"));
  } else {
    if (job->catalog) {
      jcr->impl_->res.catalog = job->catalog;
      PmStrcpy(jcr->impl_->res.catalog_source, _("Job resource"));
    } else {
      if (job->client) {
        jcr->impl_->res.catalog = job->client->catalog;
        PmStrcpy(jcr->impl_->res.catalog_source, _("Client resource"));
      } else {
  ...
  jcr->impl_->res.messages = job->messages;
  jcr->impl_->spool_data = job->spool_data;
  jcr->impl_->spool_size = job->spool_size;
  jcr->impl_->IgnoreDuplicateJobChecking = job->IgnoreDuplicateJobChecking;
  jcr->impl_->MaxRunSchedTime = job->MaxRunSchedTime;

  if (jcr->impl_->backup_format) { free(jcr->impl_->backup_format); }
  jcr->impl_->backup_format = strdup(job->backup_format);
   ...
   * This can be overridden by Console program
   */
  jcr->impl_->res.verify_job = job->verify_job;
...
void CreateClones(JobControlRecord* jcr)
{
  ...
  Dmsg2(900, "cloned=%d run_cmds=%p\n", jcr->impl_->cloned,
        jcr->impl_->res.job->run_cmds);
  if (!jcr->impl_->cloned && jcr->impl_->res.job->run_cmds) {
    char* runcmd = nullptr;
    JobId_t jobid;
    JobResource* job = jcr->impl_->res.job;
...
int CreateRestoreBootstrapFile(JobControlRecord* jcr)
{
  ...
  rx.JobIds = (char*)"";
  rx.bsr->JobId = jcr->impl_->previous_jr.JobId;
  ...
  jcr->impl_->ExpectedFiles = WriteBsrFile(ua, rx);
  if (jcr->impl_->ExpectedFiles == 0) {
    files = 0;
    goto bail_out;
  }
  FreeUaContext(ua);
  rx.bsr.reset(nullptr);
  jcr->impl_->needs_sd = true;
  return jcr->impl_->ExpectedFiles;
