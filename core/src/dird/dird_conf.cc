...
static ResourceItem dir_items[] = {
    ...
    {"DirPort", CFG_TYPE_ADDRESSES_PORT, ITEM(res_dir.DIRaddrs), 0, CFG_ITEM_DEFAULT, DIR_DEFAULT_PORT, NULL,
     NULL},
    {"DirAddress", CFG_TYPE_ADDRESSES_ADDRESS, ITEM(res_dir.DIRaddrs), 0, CFG_ITEM_DEFAULT, DIR_DEFAULT_PORT,
     NULL, NULL},
    {"DirAddresses", CFG_TYPE_ADDRESSES, ITEM(res_dir.DIRaddrs), 0, CFG_ITEM_DEFAULT, DIR_DEFAULT_PORT, NULL,
     NULL},
    {"DirSourceAddress", CFG_TYPE_ADDRESSES_ADDRESS, ITEM(res_dir.DIRsrc_addr), 0, CFG_ITEM_DEFAULT, "0", NULL,
     NULL},
    ...
    {"SubSysDirectory", CFG_TYPE_DIR, ITEM(res_dir.subsys_directory), 0, CFG_ITEM_DEPRECATED, NULL, "-12.4.0",
     NULL},
    {"MaximumConcurrentJobs", CFG_TYPE_PINT32, ITEM(res_dir.MaxConcurrentJobs), 0, CFG_ITEM_DEFAULT, "1", NULL,
     NULL},
    {"MaximumConnections", CFG_TYPE_PINT32, ITEM(res_dir.MaxConnections), 0, CFG_ITEM_DEFAULT, "30", NULL,
     NULL},
    {"MaximumConsoleConnections", CFG_TYPE_PINT32, ITEM(res_dir.MaxConsoleConnections), 0, CFG_ITEM_DEFAULT,
     "20", NULL, NULL},
    ...
    {"HeartbeatInterval", CFG_TYPE_TIME, ITEM(res_dir.heartbeat_interval), 0, CFG_ITEM_DEFAULT, "0", NULL,
     NULL},
    {"StatisticsRetention", CFG_TYPE_TIME, ITEM(res_dir.stats_retention), 0, CFG_ITEM_DEFAULT,
     "160704000" /* 5 years */, NULL, NULL},
    {"StatisticsCollectInterval", CFG_TYPE_PINT32, ITEM(res_dir.stats_collect_interval), 0, CFG_ITEM_DEFAULT,
     "150", "14.2.0-", NULL},
    {"VerId", CFG_TYPE_STR, ITEM(res_dir.verid), 0, 0, NULL, NULL, NULL},
    {"OptimizeForSize", CFG_TYPE_BOOL, ITEM(res_dir.optimize_for_size), 0, CFG_ITEM_DEFAULT, "false", NULL,
     NULL},
    {"OptimizeForSpeed", CFG_TYPE_BOOL, ITEM(res_dir.optimize_for_speed), 0, CFG_ITEM_DEFAULT, "false", NULL,
     NULL},
    {"OmitDefaults", CFG_TYPE_BOOL, ITEM(res_dir.omit_defaults), 0, CFG_ITEM_DEFAULT | CFG_ITEM_DEPRECATED,
     "true", NULL, "Omit config variables with default values when dumping the config."},
...
static ResourceItem profile_items[] = {
    ...
    {"JobACL", CFG_TYPE_ACL, ITEM(res_profile.ACL_lists), Job_ACL, 0, NULL, NULL,
     "Lists the Job resources, this resource has access to. The special keyword *all* allows access to all "
     "Job resources."},
    ...
    {"StorageACL", CFG_TYPE_ACL, ITEM(res_profile.ACL_lists), Storage_ACL, 0, NULL, NULL,
     "Lists the Storage resources, this resource has access to. The special keyword *all* allows access to "
     "all Storage resources."},
    {"ScheduleACL", CFG_TYPE_ACL, ITEM(res_profile.ACL_lists), Schedule_ACL, 0, NULL, NULL,
     "Lists the Schedule resources, this resource has access to. The special keyword *all* allows access to "
     "all Schedule resources."},
    {"PoolACL", CFG_TYPE_ACL, ITEM(res_profile.ACL_lists), Pool_ACL, 0, NULL, NULL,
     "Lists the Pool resources, this resource has access to. The special keyword *all* allows access to all "
     "Pool resources."},
    ...
    {"FileSetACL", CFG_TYPE_ACL, ITEM(res_profile.ACL_lists), FileSet_ACL, 0, NULL, NULL,
     "Lists the File Set resources, this resource has access to. The special keyword *all* allows access to "
     "all File Set resources."},
    {"CatalogACL", CFG_TYPE_ACL, ITEM(res_profile.ACL_lists), Catalog_ACL, 0, NULL, NULL,
     "Lists the Catalog resources, this resource has access to. The special keyword *all* allows access to "
...
static ResourceItem cli_items[] = {
    ...
    {"ConnectionFromDirectorToClient", CFG_TYPE_BOOL, ITEM(res_client.conn_from_dir_to_fd), 0,
     CFG_ITEM_DEFAULT, "true", "16.2.2", "Let the Director initiate the network connection to the Client."},
    {"AllowClientConnect", CFG_TYPE_BOOL, ITEM(res_client.conn_from_fd_to_dir), 0,
     CFG_ITEM_DEPRECATED | CFG_ITEM_ALIAS, NULL, NULL, "Alias of \"Connection From Client To Director\"."},
    ...
    {"QuotaIncludeFailedJobs", CFG_TYPE_BOOL, ITEM(res_client.QuotaIncludeFailedJobs), 0, CFG_ITEM_DEFAULT,
     "true", NULL, NULL},
    ...
    {"NdmpBlockSize", CFG_TYPE_SIZE32, ITEM(res_client.ndmp_blocksize), 0, CFG_ITEM_DEFAULT, "64512", NULL,
     NULL},
...
static ResourceItem store_items[] = {
    ...
    {"Protocol", CFG_TYPE_AUTHPROTOCOLTYPE, ITEM(res_store.Protocol), 0, CFG_ITEM_DEFAULT, "Native", NULL,
     NULL},
    ...
    {"Enabled", CFG_TYPE_BOOL, ITEM(res_store.enabled), 0, CFG_ITEM_DEFAULT, "true", NULL,
     "En- or disable this resource."},
    {"AllowCompression", CFG_TYPE_BOOL, ITEM(res_store.AllowCompress), 0, CFG_ITEM_DEFAULT, "true", NULL,
     NULL},
    {"HeartbeatInterval", CFG_TYPE_TIME, ITEM(res_store.heartbeat_interval), 0, CFG_ITEM_DEFAULT, "0", NULL,
     NULL},
    {"CacheStatusInterval", CFG_TYPE_TIME, ITEM(res_store.cache_status_interval), 0, CFG_ITEM_DEFAULT, "30",
     NULL, NULL},
    {"MaximumConcurrentJobs", CFG_TYPE_PINT32, ITEM(res_store.MaxConcurrentJobs), 0, CFG_ITEM_DEFAULT, "1",
     NULL, NULL},
    {"MaximumConcurrentReadJobs", CFG_TYPE_PINT32, ITEM(res_store.MaxConcurrentReadJobs), 0, CFG_ITEM_DEFAULT,
     "0", NULL, NULL},
    ...
    {"CollectStatistics", CFG_TYPE_BOOL, ITEM(res_store.collectstats), 0, CFG_ITEM_DEFAULT, "false", NULL,
     NULL},
    {"NdmpChangerDevice", CFG_TYPE_STRNAME, ITEM(res_store.ndmp_changer_device), 0, 0, NULL, "16.2.4-",
     "Allows direct control of a Storage Daemon Auto Changer device by the Director. Only used in NDMP_NATIVE "
     "environments."},
    ...
    //  "Allows direct control of Storage Daemon Tape devices by the Director. Only used in NDMP_NATIVE
    //  environments." },
...
static ResourceItem cat_items[] = {
    ...
    {"DisableBatchInsert", CFG_TYPE_BOOL, ITEM(res_cat.disable_batch_insert), 0, CFG_ITEM_DEFAULT, "false",
     NULL, NULL},
    {"Reconnect", CFG_TYPE_BOOL, ITEM(res_cat.try_reconnect), 0, CFG_ITEM_DEFAULT, "false", "15.1.0-",
     "Try to reconnect a database connection when its dropped"},
    {"ExitOnFatal", CFG_TYPE_BOOL, ITEM(res_cat.exit_on_fatal), 0, CFG_ITEM_DEFAULT, "false", "15.1.0-",
...
ResourceItem job_items[] = {
    ...
    {"FullMaxWaitTime", CFG_TYPE_TIME, ITEM(res_job.FullMaxRunTime), 0, CFG_ITEM_DEPRECATED, NULL, "-12.4.0",
     NULL},
    ...
    {"PurgeMigrationJob", CFG_TYPE_BOOL, ITEM(res_job.PurgeMigrateJob), 0, CFG_ITEM_DEFAULT, "false", NULL,
     NULL},
    {"Enabled", CFG_TYPE_BOOL, ITEM(res_job.enabled), 0, CFG_ITEM_DEFAULT, "true", NULL,
     "En- or disable this resource."},
    {"SpoolAttributes", CFG_TYPE_BOOL, ITEM(res_job.SpoolAttributes), 0, CFG_ITEM_DEFAULT, "false", NULL,
     NULL},
    ...
    {"RerunFailedLevels", CFG_TYPE_BOOL, ITEM(res_job.rerun_failed_levels), 0, CFG_ITEM_DEFAULT, "false", NULL,
     NULL},
    ...
    {"MaximumConcurrentJobs", CFG_TYPE_PINT32, ITEM(res_job.MaxConcurrentJobs), 0, CFG_ITEM_DEFAULT, "1", NULL,
     NULL},
    {"RescheduleOnError", CFG_TYPE_BOOL, ITEM(res_job.RescheduleOnError), 0, CFG_ITEM_DEFAULT, "false", NULL,
     NULL},
    ...
    {"AllowDuplicateJobs", CFG_TYPE_BOOL, ITEM(res_job.AllowDuplicateJobs), 0, CFG_ITEM_DEFAULT, "true", NULL,
     NULL},
    ...
    {"CancelQueuedDuplicates", CFG_TYPE_BOOL, ITEM(res_job.CancelQueuedDuplicates), 0, CFG_ITEM_DEFAULT,
     "false", NULL, NULL},
    {"CancelRunningDuplicates", CFG_TYPE_BOOL, ITEM(res_job.CancelRunningDuplicates), 0, CFG_ITEM_DEFAULT,
     "false", NULL, NULL},
    {"SaveFileHistory", CFG_TYPE_BOOL, ITEM(res_job.SaveFileHist), 0, CFG_ITEM_DEFAULT, "true", "14.2.0-",
     NULL},
    {"FileHistorySize", CFG_TYPE_SIZE64, ITEM(res_job.FileHistSize), 0, CFG_ITEM_DEFAULT, "10000000",
     "15.2.4-", NULL},
    ...
    {"MaxConcurrentCopies", CFG_TYPE_PINT32, ITEM(res_job.MaxConcurrentCopies), 0, CFG_ITEM_DEFAULT, "100",
     NULL, NULL},
    /* Settings for always incremental */
    {"AlwaysIncremental", CFG_TYPE_BOOL, ITEM(res_job.AlwaysIncremental), 0, CFG_ITEM_DEFAULT, "false",
     "16.2.4-", "Enable/disable always incremental backup scheme."},
    ...
    {"AlwaysIncrementalKeepNumber", CFG_TYPE_PINT32, ITEM(res_job.AlwaysIncrementalKeepNumber), 0,
     CFG_ITEM_DEFAULT, "0", "16.2.4-",
     "Guarantee that at least the specified number of Backup Jobs will persist, even if they are older than "
     "\"Always Incremental Job Retention\"."},
    {"AlwaysIncrementalMaxFullAge", CFG_TYPE_TIME, ITEM(res_job.AlwaysIncrementalMaxFullAge), 0, 0, NULL,
     "16.2.4-",
     "If \"AlwaysIncrementalMaxFullAge\" is set, during consolidations only incremental backups will be "
     "considered while the Full Backup remains to reduce the amount of data being consolidated. Only if the "
     "Full Backup is older than \"AlwaysIncrementalMaxFullAge\", the Full Backup will be part of the "
     "consolidation to avoid the Full Backup becoming too old ."},
    {"MaxFullConsolidations", CFG_TYPE_PINT32, ITEM(res_job.MaxFullConsolidations), 0, CFG_ITEM_DEFAULT, "0",
     "16.2.4-",
...
static ResourceItem fs_items[] = {
    ...
    {"IgnoreFileSetChanges", CFG_TYPE_BOOL, ITEM(res_fs.ignore_fs_changes), 0, CFG_ITEM_DEFAULT, "false", NULL,
     NULL},
...
static ResourceItem pool_items[] = {
    ...
    {"CleaningPrefix", CFG_TYPE_STRNAME, ITEM(res_pool.cleaning_prefix), 0, CFG_ITEM_DEFAULT, "CLN", NULL,
     NULL},
    {"UseCatalog", CFG_TYPE_BOOL, ITEM(res_pool.use_catalog), 0, CFG_ITEM_DEFAULT, "true", NULL, NULL},
    {"UseVolumeOnce", CFG_TYPE_BOOL, ITEM(res_pool.use_volume_once), 0, CFG_ITEM_DEPRECATED, NULL, "-12.4.0",
     NULL},
    {"PurgeOldestVolume", CFG_TYPE_BOOL, ITEM(res_pool.purge_oldest_volume), 0, CFG_ITEM_DEFAULT, "false",
     NULL, NULL},
    {"ActionOnPurge", CFG_TYPE_ACTIONONPURGE, ITEM(res_pool.action_on_purge), 0, 0, NULL, NULL, NULL},
    {"RecycleOldestVolume", CFG_TYPE_BOOL, ITEM(res_pool.recycle_oldest_volume), 0, CFG_ITEM_DEFAULT, "false",
     NULL, NULL},
    {"RecycleCurrentVolume", CFG_TYPE_BOOL, ITEM(res_pool.recycle_current_volume), 0, CFG_ITEM_DEFAULT,
     "false", NULL, NULL},
 ...
 * name token
 */
struct s_kw ActionOnPurgeOptions[] = {{"None", ON_PURGE_NONE}, {"Truncate", ON_PURGE_TRUNCATE}, {NULL, 0}};
...
json_t *json_datatype_header(const int type, const char *typeclass)
{
  ...
  if (description) { json_object_set_new(json, "description", json_string(description)); }
...
json_t *json_datatype(const int type, s_kw items[])
{
  ...
  if (items) {
    ...
    for (int i = 0; items[i].name; i++) { json_object_set_new(values, items[i].name, json_item(&items[i])); }
...
const char *get_configure_usage_string()
{
  ...
  if (!configure_usage_string) { configure_usage_string = new PoolMem(PM_BSOCK); }
    ...
    if (!BitIsSet(i, dest->hdr.item_present) && BitIsSet(i, source->hdr.item_present)) {
      ...
      switch (items[i].type) {
        ...
        case CFG_TYPE_RES: {
          ...
          def_svalue = (char **)((char *)(source) + offset);
          svalue     = (char **)((char *)dest + offset);
          if (*svalue) { Pmsg1(000, _("Hey something is wrong. p=0x%lu\n"), *svalue); }
          *svalue = *def_svalue;
          SetBit(i, dest->hdr.item_present);
          SetBit(i, dest->hdr.inherit_content);
          break;
...
bool JobResource::validate()
{
  ...
  switch (JobType) {
    ...
    default:
       ...
       */
      if (!client) {
        Jmsg(NULL, M_ERROR, 0, _("\"client\" directive in Job \"%s\" resource is required, but not found.\n"),
             name());
        return false;
      }

      if (!fileset) {
        Jmsg(NULL, M_ERROR, 0, _("\"fileset\" directive in Job \"%s\" resource is required, but not found.\n"),
             name());
        return false;
      }
...
char *CatalogResource::display(POOLMEM *dst)
{
  Mmsg(dst,
       ...
       "db_password=%s\ndb_address=%s\ndb_port=%i\n"
       "db_socket=%s\n",
       name(), NPRTB(db_name), NPRTB(db_driver), NPRTB(db_user), NPRTB(db_password.value), NPRTB(db_address),
       db_port, NPRTB(db_socket));
...
static inline void PrintConfigRun(ResourceItem *item, PoolMem &cfg_str)
{
  ...
  char *months[]   = {(char *)"Jan", (char *)"Feb", (char *)"Mar", (char *)"Apr", (char *)"May", (char *)"Jun",
  ...
  if (run != NULL) {
    while (run) {
      ...
      if (!all_set) {
        ...
        /*
         * See if we are still in an interval and the last bit is also set then the interval stretches to the
         * last item.
         */
        i = nr_items - 1;
...
const char *auth_protocol_to_str(uint32_t auth_protocol)
{
  for (int i = 0; authprotocols[i].name; i++) {
    if (authprotocols[i].token == auth_protocol) { return authprotocols[i].name; }
  }
    ...
    case R_JOB:
      if (!(res = (UnionOfResources *)my_config->GetResWithName(type, res_all.res_job.name()))) {
        ...
        if (!res->res_job.RegexWhere &&
          ...
          bregexp_build_where(res->res_job.RegexWhere, len, res->res_job.strip_prefix, res->res_job.add_prefix,
                              res->res_job.add_suffix);
          /*
           * TODO: test bregexp
           */
...
bool PropagateJobdefs(int res_type, JobResource *res)
{
  ...
  if (res_type == R_JOB) {
    ...
    if (jobdefs->RunScripts) {
      ...
      if (!res->RunScripts) { res->RunScripts = New(alist(10, not_owned_by_alist)); }
...
static void StorePooltype(LEX *lc, ResourceItem *item, int index, int pass)
{
  ...
  if (pass == 1) {
    ...
    if (i != 0) { scan_err1(lc, _("Expected a Pool Type option, got: %s"), lc->str); }
  }
...
static void StoreActiononpurge(LEX *lc, ResourceItem *item, int index, int pass)
{
  ...
  if (i != 0) { scan_err1(lc, _("Expected an Action On Purge option, got: %s"), lc->str); }
...
static void StoreMigtype(LEX *lc, ResourceItem *item, int index, int pass)
{
  ...
  if (i != 0) { scan_err1(lc, _("Expected a Migration Job Type keyword, got: %s"), lc->str); }
...
static void StoreJobtype(LEX *lc, ResourceItem *item, int index, int pass)
{
  ...
  if (i != 0) { scan_err1(lc, _("Expected a Job Type keyword, got: %s"), lc->str); }
...
static void StoreProtocoltype(LEX *lc, ResourceItem *item, int index, int pass)
{
  ...
  if (i != 0) { scan_err1(lc, _("Expected a Protocol Type keyword, got: %s"), lc->str); }
...
static void StoreReplace(LEX *lc, ResourceItem *item, int index, int pass)
{
  ...
  if (i != 0) { scan_err1(lc, _("Expected a Restore replacement option, got: %s"), lc->str); }
...
static void StoreAuthprotocoltype(LEX *lc, ResourceItem *item, int index, int pass)
{
  ...
  if (i != 0) { scan_err1(lc, _("Expected a Auth Protocol Type keyword, got: %s"), lc->str); }
  ScanToEol(lc);
  SetBit(index, res_all.hdr.item_present);
  ClearBit(index, res_all.hdr.inherit_content);
}
...
static void StoreLevel(LEX *lc, ResourceItem *item, int index, int pass)
{
  ...
  if (i != 0) { scan_err1(lc, _("Expected a Job Level keyword, got: %s"), lc->str); }
...
static void StoreRunscript(LEX *lc, ResourceItem *item, int index, int pass)
{
  ...
  if (token != BCT_BOB) { scan_err1(lc, _("Expecting open brace. Got %s"), lc->str); }
  /*
  ...
  while ((token = LexGetToken(lc, BCT_SKIP_EOL)) != BCT_EOF) {
    ...
    if (token != BCT_IDENTIFIER) { scan_err1(lc, _("Expecting keyword, got: %s\n"), lc->str); }

    for (i = 0; runscript_items[i].name; i++) {
      if (Bstrcasecmp(runscript_items[i].name, lc->str)) {
        token = LexGetToken(lc, BCT_SKIP_EOL);
        if (token != BCT_EQUALS) { scan_err1(lc, _("Expected an equals, got: %s"), lc->str); }
    ...
    if (i >= 0) { scan_err1(lc, _("Keyword %s not permitted in this resource"), lc->str); }
  }
        ...
        for (int j = 0; backupprotocols[j].name; j++) {
          if (backupprotocols[j].token == protocol) {
            ...
            if (items[i].flags & CFG_ITEM_DEFAULT) {
              if (Bstrcasecmp(items[i].default_value, backupprotocols[j].name)) { break; }
            }
      ...
      if (replace) {
        for (int j = 0; ReplaceOptions[j].name; j++) {
          if (ReplaceOptions[j].token == replace) {
             ...
             */
            if (items[i].flags & CFG_ITEM_DEFAULT) {
              if (Bstrcasecmp(items[i].default_value, ReplaceOptions[j].name)) { break; }
            }
