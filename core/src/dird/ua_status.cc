...
bool DotStatusCmd(UaContext* ua, const char* cmd)
{
  ...
  if (Bstrcasecmp(ua->argk[1], "dir")) {
    if (Bstrcasecmp(ua->argk[2], "current")) {
      ...
      foreach_jcr (njcr) {
        if (njcr->JobId != 0 &&
            ua->AclAccessOk(Job_ACL, njcr->impl_->res.job->resource_name_)) {
...
static void PrtRuntime(UaContext* ua, sched_pkt* sp)
{
  ...
  if (sp->job->JobType == JT_BACKUP) {
    ...
    if (ok) {
      ...
      jcr->impl_->res.write_storage = sp->store;
...
static void ListRunningJobs(UaContext* ua)
{
  ...
  foreach_jcr (jcr) {
    if (jcr->JobId == 0 ||
        !ua->AclAccessOk(Job_ACL, jcr->impl_->res.job->resource_name_)) {
      continue;
    }
    njobs++;
      ...
      case JS_WaitFD:
        ...
        if (!jcr->file_bsock) {
          ...
          Mmsg(emsg, _("is waiting for Client to connect (Client Initiated "
                       "Connection)"));
        } else if (!jcr->impl_->res.client || !jcr->impl_->res.write_storage) {
