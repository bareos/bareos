...
static void HandleGenerateScript(WsCodec& ws, json_t* msg)
{
  ...
  tape.enabled = JBool(msg, "tape_enabled");
  for (const auto& assignment : ParseTapeAssignments(msg)) {
    tape.assignments.push_back({assignment.changer_path, assignment.drive_paths});
  }
  ...
  const auto add_repo = BuildAddRepoCmd(JStr(msg, "distro"), JStr(msg, "version"),
                                        JStr(msg, "repo_type"),
                                        JStr(msg, "repo_login"),
                                        JStr(msg, "repo_password"));
  ...
  const auto install = BuildInstallCmd(JStr(msg, "pkg_mgr"), BuildDefaultPackageList());
