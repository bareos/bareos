...
TEST(BconfigService, PersistsStorageBootstrapDiscoveryByIdSerialFallback)
{
  ScopedDirectory state_path{MakeTempPath()};
  ScopedDirectory repo_path{MakeTempPath()};
  ...
  const auto session_base_path
      = std::string{"/v1/bootstrap/storage-sessions/"} + session_id_text;
  const auto by_id_tape = std::string{"/dev/tape/by-id/scsi-4e77fe415f"};
  ...
  const auto discovery_response = server.Post(
      ...
      "{\"bootstrap_token\":\"" + bootstrap_token_text
          + "\",\"hostname\":\"sdhost\",\"fqdn\":\"sdhost.example.com\","
            "\"report\":{\"hostname\":\"sdhost\",\"filesystems\":[],"
            "\"tape_devices\":[{\"device_node\":\"/dev/tape/by-id/scsi-4e77fe415f\","
            "\"generic_device_node\":\"/dev/sg3\",\"vendor\":\"HPE\","
            "\"model\":\"Ultrium 9-SCSI\",\"device_identifier\":\"naa.2034453737343146\","
            "\"serial\":\"4E77FE415F\",\"accessible\":true,"
            "\"accessibility_error\":\"\"}],"
             "\"changers\":[{\"device_node\":\"/dev/sg4\",\"vendor\":\"IBM\","
             "\"model\":\"3573-TL\",\"device_identifier\":\"naa.aabbccdd\","
             "\"serial\":\"CHANGER42\","
             "\"drive_device_nodes\":[\"/dev/tape/by-id/scsi-4e77fe415f\"],"
              "\"drives\":[{\"tape_device_node\":\"/dev/tape/by-id/scsi-4e77fe415f\","
              "\"generic_device_node\":\"/dev/sg3\","
              "\"drive_element_address\":256,"
  ...
  EXPECT_STREQ(
      json_string_value(json_object_get(loaded_drive, "tape_device_node")),
      by_id_tape.c_str());
  EXPECT_STREQ(json_string_value(json_object_get(loaded_drive, "source")),
               "read_element_status:serial");
#endif
}

TEST(BconfigService, PersistsStorageBootstrapDiscoveryByIdScsiFallback)
{
  ScopedDirectory state_path{MakeTempPath()};
  ScopedDirectory repo_path{MakeTempPath()};
  ...
  ASSERT_TRUE(json_is_object(loaded_drive));
  EXPECT_EQ(json_integer_value(
                json_object_get(loaded_drive, "drive_element_address")),
            258);
  EXPECT_STREQ(
      json_string_value(json_object_get(loaded_drive, "tape_device_node")),
      by_id_tape.c_str());
  EXPECT_STREQ(
      json_string_value(json_object_get(loaded_drive, "device_identifier")),
      "naa.55667788");
  EXPECT_STREQ(json_string_value(json_object_get(loaded_drive, "serial")),
               "TAPE123");
  EXPECT_STREQ(json_string_value(json_object_get(loaded_drive, "source")),
