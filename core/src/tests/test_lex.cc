...
TEST(LexTokenConstantsTest, BctAllIsZero) { EXPECT_EQ(BCT_ALL, 0); }

TEST(LexTokenConstantsTest, AllTokensDistinct)
{
  std::vector<int> tokens
      = {BCT_NONE,          BCT_EOF,        BCT_NUMBER,
         BCT_IPADDR,        BCT_IDENTIFIER, BCT_UNQUOTED_STRING,
         BCT_QUOTED_STRING, BCT_BOB,        BCT_EOB,
         BCT_EQUALS,        BCT_COMMA,      BCT_EOL,
         BCT_ERROR};
...
TEST(LexStateTest, NoneIsZero) { EXPECT_EQ(lex_none, 0); }
