...
TEST(crc32, internal_spd)
{
  ...
  std::iota(buf, buf + len, 0xbb);

  for (int i = 0; i < 10000; i++) {
    ASSERT_EQ(0xcf6610d8, crc32_fast(buf, len));
  }
  free(buf);
}

TEST(crc32, chksum_label)
{
  /* the first four bytes of a block contain the crc32 and are not part of the
   * checksum, so we skip them
   */
  ASSERT_EQ(0xcb678ddd, crc32_fast(label_block + 4, 200));
}
