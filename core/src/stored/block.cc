...
bool DeviceControlRecord::WriteBlockToDev()
{
  ...
  do {
    if (retry > 0 && status == -1 && errno == EBUSY) {
      BErrNo be;
      Dmsg4(100, "===== write retry=%d status=%zd errno=%d: ERR=%s\n", retry,
  ...
  if (status != (ssize_t)wlen) {
    ...
    if (debug_level >= 100) {
      ...
      be.SetErrno(dev->dev_errno);
      Dmsg7(100,
            "=== Write error. fd=%d size=%u rtn=%zd dev_blk=%d blk_blk=%d "
            "errno=%d: ERR=%s\n",
...
reread:
  ...
  do {
    if (retry) {
      BErrNo be;
      Dmsg4(100, "===== read retry=%d status=%zd errno=%d: ERR=%s\n", retry,
            status, errno, be.bstrerror());
  ...
  Dmsg3(250, "Read device got %zd bytes at %u:%u\n", status, dev->file,
        dev->block_num);
