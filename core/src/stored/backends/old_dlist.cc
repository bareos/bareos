 ...
 *
 * old_dlist is a doubly linked list with the links being in the list data item.
 *
 ...
 * Init old_dlist
 */
void old_dlist::init(void* item, old_dlink* link)
{
  head = tail = NULL;
  loffset = (int)((char*)link - (char*)item);
  if (loffset < 0 || loffset > 5000) {
    Emsg0(M_ABORT, 0, "Improper old_dlist initialization.\n");
  }
  num_items = 0;
}
 ...
 */
void old_dlist::append(void* item)
{
  SetNext(item, NULL);
  SetPrev(item, tail);
 ...
 */
void old_dlist::prepend(void* item)
{
  SetNext(item, head);
  SetPrev(item, NULL);
...
void old_dlist::InsertBefore(void* item, void* where)
{
  old_dlink* where_link = get_link(where);
...
void old_dlist::InsertAfter(void* item, void* where)
{
  old_dlink* where_link = get_link(where);
 ...
 */
void* old_dlist::binary_insert(void* item,
 ...
 *  or not.
 */
void old_dlist::BinaryInsertMultiple(void* item,
                                     int compare(void* item1, void* item2))
{
 ...
 * Search for item
 */
void* old_dlist::binary_search(void* item,
                               int compare(void* item1, void* item2))
{
  int comp;
  int low, high, cur;
...
void old_dlist::remove(void* item)
{
  void* xitem;
  old_dlink* ilink = get_link(item); /* item's link */
  if (item == head) {
    head = ilink->next;
...
void* old_dlist::next(void* item)
{
  if (item == NULL) { return head; }
  return get_next(item);
}

void* old_dlist::prev(void* item)
{
...
/* String helpers for old_dlist usage */

old_dlistString* new_old_dlistString(const char* str)
{
  return new_old_dlistString(str, strlen(str));
}

old_dlistString* new_old_dlistString(const char* str, int len)
{
  old_dlistString* node;
  node = (old_dlistString*)malloc(sizeof(old_dlink) + len + 1);
