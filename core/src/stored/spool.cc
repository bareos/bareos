...
static int ReadBlockFromSpoolFile(DeviceControlRecord* dcr)
{
  ...
  if (status == 0) {
    ...
    if (status == -1) {
      ...
      Pmsg2(000, T_("Spool read error. Wanted %u bytes, got %zd\n"), rlen,
            status);
      Jmsg2(jcr, M_FATAL, 0,
            T_("Spool header read error. Wanted %u bytes, got %zd\n"), rlen,
            status);
    }
    jcr->setJobStatus(JS_FatalError); /* override any Incomplete */
    return RB_ERROR;
  ...
  if (status != (ssize_t)rlen) {
    Pmsg2(000, T_("Spool data read error. Wanted %u bytes, got %zd\n"), rlen,
          status);
    Jmsg2(dcr->jcr, M_FATAL, 0,
          T_("Spool data read error. Wanted %u bytes, got %zd\n"), rlen,
          status);
    jcr->setJobStatus(JS_FatalError); /* override any Incomplete */
    return RB_ERROR;
  }
