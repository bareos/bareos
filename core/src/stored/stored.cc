...
static int CheckResources()
{
   ...
   tls_needed = store->tls_cert.enabled || store->tls_cert.authenticate;
   ...
   foreach_res(director, R_DIRECTOR) {
      /* tls_require implies tls_enable */
      if (director->tls_cert.required) {
         director->tls_cert.enabled = true;
      }

      tls_needed = director->tls_cert.enabled || director->tls_cert.authenticate;
