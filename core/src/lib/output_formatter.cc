...
void OutputFormatter::ObjectKeyValue(const char *key, const char *value, const char *value_fmt, int wrap)
{
...
{
  if (!hidden_columns || column < 0 || column > OF_MAX_NR_HIDDEN_COLUMNS) { return false; }
...
{
  if (hidden_columns) { ClearAllBits(OF_MAX_NR_HIDDEN_COLUMNS, hidden_columns); }
}
...
bool OutputFormatter::JsonKeyValueAddBool(const char *key, bool value)
{
  ...
  if (json_obj == NULL) { Emsg2(M_ERROR, 0, "No json object defined to add %s: %llu", key, value); }
