...
struct Digest {
 ...
 public:
  Digest(JobControlRecord *jcr, crypto_digest_t type) : jcr(jcr), type(type) { EVP_MD_CTX_init(&ctx); }
...
bool CryptoKeypairHasKey(const char *file)
{
  ...
  while (PEM_read_bio(bio, &name, &header, &data, &len)) {
     ...
     */
    if (bstrcmp(name, PEM_STRING_RSA) || bstrcmp(name, PEM_STRING_DSA) || bstrcmp(name, PEM_STRING_PKCS8) ||
 ...
 *          NULL on failure.
 * Returns: CRYPTO_ERROR_NONE and a pointer to a newly allocated CRYPTO_SESSION structure in *session on
 * success. A crypto_error_t value on failure.
 */
...
CIPHER_CONTEXT *crypto_cipher_new(CRYPTO_SESSION *cs, bool encrypt, uint32_t *blocksize)
{
  ...
  if (!EVP_CipherInit_ex(cipher_ctx->ctx, NULL, NULL, cs->session_key, M_ASN1_STRING_data(cs->cryptoData->iv),
                         -1)) {
...
bool CryptoCipherUpdate(CIPHER_CONTEXT *cipher_ctx,
                        ...
                        uint32_t *written)
{
  if (!EVP_CipherUpdate(cipher_ctx->ctx, (unsigned char *)dest, (int *)written, (const unsigned char *)data,
                        length)) {
...
int InitCrypto(void)
{
  ...
  if (!OpensslSeedPrng()) { Jmsg0(NULL, M_ERROR_TERM, 0, _("Failed to seed OpenSSL PRNG\n")); }
...
int CleanupCrypto(void)
{
  ...
  if (!OpensslSavePrng()) { Jmsg0(NULL, M_ERROR, 0, _("Failed to save OpenSSL PRNG\n")); }
 ...
 */
void OpensslPostErrors(int type, const char *errstring) { OpensslPostErrors(NULL, type, errstring); }
