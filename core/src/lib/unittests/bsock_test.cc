...

{
  ...
  if (bs->recv() <= 0) {
    ...
    if (!bs->AuthenticateInboundConnection(NULL, "Console", name, *password, dir_cons_config.get())) {
      ...
      if (dir_cons_config->tls_psk.enabled || dir_cons_config->tls_cert.enabled) {
...
#endif
{
  ...
  if (!UA_sock->connect(NULL, 1, 15, heart_beat, "Director daemon", (char *)server_address.c_str(),
    ...
    if (!UA_sock->AuthenticateWithDirector(&jcr, name, *password, errmsg, errmsg_len, cons_dir_config.get())) {
      ...
      if (cons_dir_config->tls_psk.enabled || cons_dir_config->tls_cert.enabled) {
...
TEST(bsock, auth_works)
{
  ...
  cons_dir_config->tls_psk.enabled = false;
  dir_cons_config->tls_psk.enabled = false;
...
TEST(bsock, auth_works_with_tls_cert)
{
  ...
  cons_dir_config->tls_psk.enabled = true;
  cons_dir_config->tls_cert.enabled = true;
  dir_cons_config->tls_cert.enabled = true;
