...
bool SendPluginName(JobControlRecord* jcr, BareosSocket* sd, bool start)
{
  ...
  struct save_pkt* sp = (struct save_pkt*)jcr->impl_->plugin_sp;
    ...
    switch (var) {
      ...
      case bVarVssClient:
        ...
        if (jcr->impl_->pVSSClient) {
          *(void**)value = jcr->impl_->pVSSClient;
...
static bRC bareosAddExclude(bpContext* ctx, const char* fname)
{
  ...
  if (!bctx->exclude) { bctx->exclude = new_exclude(jcr->impl_->ff->fileset); }
...
static bRC bareosNewOptions(bpContext* ctx)
{
  ...
  if (!IsCtxGood(ctx, jcr, bctx)) { return bRC_Error; }
  (void)NewOptions(jcr->impl_->ff, jcr->impl_->ff->fileset->incexe);
...
static bRC bareosNewPreInclude(bpContext* ctx)
{
  ...
  bctx->include = new_preinclude(jcr->impl_->ff->fileset);
  NewOptions(jcr->impl_->ff, bctx->include);
  SetIncexe(jcr, bctx->include);
