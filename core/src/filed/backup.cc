                              ...
                              crypto_cipher_t cipher)
{
  ...
  if (have_acl) {
    ...
    jcr->impl_->acl_data->u.build =
        (acl_build_data_t*)malloc(sizeof(acl_build_data_t));
    memset(jcr->impl_->acl_data->u.build, 0, sizeof(acl_build_data_t));
    jcr->impl_->acl_data->u.build->content = GetPoolMemory(PM_MESSAGE);
  }

  if (have_xattr) {
    jcr->impl_->xattr_data = (xattr_data_t*)malloc(sizeof(xattr_data_t));
    memset(jcr->impl_->xattr_data, 0, sizeof(xattr_data_t));
    jcr->impl_->xattr_data->u.build =
        (xattr_build_data_t*)malloc(sizeof(xattr_build_data_t));
    memset(jcr->impl_->xattr_data->u.build, 0, sizeof(xattr_build_data_t));
    jcr->impl_->xattr_data->u.build->content = GetPoolMemory(PM_MESSAGE);
  }

  /**
   * Subroutine SaveFile() is called for each file
   */
  if (!FindFiles(jcr, (FindFilesPacket*)jcr->impl_->ff, SaveFile, PluginSave)) {
    ok = false; /* error */
    jcr->setJobStatus(JS_ErrorTerminated);
  }

  if (have_acl && jcr->impl_->acl_data->u.build->nr_errors > 0) {
    Jmsg(jcr, M_WARNING, 0,
         _("Encountered %ld acl errors while doing backup\n"),
         jcr->impl_->acl_data->u.build->nr_errors);
  }
  if (have_xattr && jcr->impl_->xattr_data->u.build->nr_errors > 0) {
    Jmsg(jcr, M_WARNING, 0,
         _("Encountered %ld xattr errors while doing backup\n"),
         jcr->impl_->xattr_data->u.build->nr_errors);
  }
  ...
  if (have_acl && jcr->impl_->acl_data) {
    FreePoolMemory(jcr->impl_->acl_data->u.build->content);
    free(jcr->impl_->acl_data->u.build);
    free(jcr->impl_->acl_data);
    jcr->impl_->acl_data = NULL;
  }

  if (have_xattr && jcr->impl_->xattr_data) {
    FreePoolMemory(jcr->impl_->xattr_data->u.build->content);
    free(jcr->impl_->xattr_data->u.build);
    free(jcr->impl_->xattr_data);
    jcr->impl_->xattr_data = NULL;
  }

  if (jcr->impl_->big_buf) {
    free(jcr->impl_->big_buf);
    jcr->impl_->big_buf = NULL;
  }
...
static inline bool SetupEncryptionDigests(b_save_ctx& bsctx)
{
   ...
   */
  if (bsctx.jcr->impl_->crypto.pki_sign) {
   ...
   * Enable encryption
   */
  if (bsctx.jcr->impl_->crypto.pki_encrypt) {
...
static inline bool TerminateSigningDigest(b_save_ctx& bsctx)
{
  ...
  if (!CryptoSignAddSigner(signature, bsctx.signing_digest,
                           bsctx.jcr->impl_->crypto.pki_keypair)) {
    Jmsg(bsctx.jcr, M_FATAL, 0,
...
static inline bool DoBackupAcl(JobControlRecord* jcr, FindFilesPacket* ff_pkt)
{
  ...
  jcr->impl_->acl_data->filetype = ff_pkt->type;
  jcr->impl_->acl_data->last_fname = jcr->impl_->last_fname;

  if (jcr->IsPlugin()) {
    retval = PluginBuildAclStreams(jcr, jcr->impl_->acl_data, ff_pkt);
  } else {
    retval = BuildAclStreams(jcr, jcr->impl_->acl_data, ff_pkt);
  }
...
static inline bool DoBackupXattr(JobControlRecord* jcr, FindFilesPacket* ff_pkt)
{
  ...
  if (jcr->IsPlugin()) {
    retval = PluginBuildXattrStreams(jcr, jcr->impl_->xattr_data, ff_pkt);
  } else {
    retval = BuildXattrStreams(jcr, jcr->impl_->xattr_data, ff_pkt);
  }
...
int SaveFile(JobControlRecord* jcr, FindFilesPacket* ff_pkt, bool top_level)
{
  ...
  jcr->impl_->num_files_examined++; /* bump total file count */
  ...
  switch (ff_pkt->type) {
    ...
    case FT_PLUGIN_CONFIG:
      ...
      break;
    case FT_DIRBEGIN:
      jcr->impl_->num_files_examined--; /* correct file count */
      return 1;                         /* not used */
    case FT_NORECURSE:
      Jmsg(jcr, M_INFO, 1,
...
bail_out:
  ...
  if (ff_pkt->opt_plugin) {
    jcr->impl_->plugin_sp = NULL; /* sp is local to this function */
                     ...
                     DIGEST* signing_digest)
{
  ...
  if (sd->message_length < 0) { /* error */
    ...
    if (!CryptoCipherFinalize(bctx.cipher_ctx,
                              (uint8_t*)jcr->impl_->crypto.crypto_buf,
                              &bctx.encrypted_len)) {
      /*
    ...
    if (bctx.encrypted_len > 0) {
      ...
      sd->msg = jcr->impl_->crypto.crypto_buf; /* set correct write buffer */
      if (!sd->send()) {
        if (!jcr->IsJobCanceled()) {
          Jmsg1(jcr, M_FATAL, 0, _("Network send error to SD. ERR=%s\n"),
                sd->bstrerror());
                             ...
                             int& data_stream)
{
  ...
  PmStrcpy(jcr->impl_->last_fname, ff_pkt->fname);
  jcr->unlock();
...
static void CloseVssBackupSession(JobControlRecord* jcr)
{
  ...
  if (jcr->impl_->pVSSClient) {
    ...
    if (jcr->impl_->pVSSClient->CloseBackup()) {
      /*
     ...
     */
    wchar_t* metadata = jcr->impl_->pVSSClient->GetMetadata();
    if (metadata) {
      FindFilesPacket* ff_pkt = jcr->impl_->ff;
