...
still have to be undertaken mostly manually. For FreeBSD the
same procedures may be used but they are not yet developed.  For Windows,
a number of Bareos users have reported success using BartPE.
...
ReaR supports Bareos as backend type since version 1.15. Make sure you get at
least that release.
ReaR is included in some Linux distributions.
If you don't find ReaR \textgreater= 1.15 in your distribution, check
the \elink{download section on the
...
There is one important requirement for using Bareos with ReaR regarding your
Bareos configuration: The Bareos client name (name of the file daemon resource)
must start with the hostname of the system. This requirement will likely
be removed in the future. In our example we use \host{myclient} as a
hostname and \parameter{myclient-fd} as a Bareos client name.
...
for details
(\elink{http://relax-and-recover.org/documentation/}{
http://relax-and-recover.org/documentation/}).

{\bf Complete Configuration Example}: For example, if you want to create an ISO
image and store it to an NFS server
with the IP Address 192.168.10.1, you can use the following configuration:

\begin{config}{/etc/rear/local.conf}
# This is default:
#OUTPUT=ISO
...
<command></command><parameter>rear -v mkrescue</parameter>
\end{commands}
...
other means, e.g. by a regular backup schedule or by using bconsole (the same
applies for the "rear mkbackup" command).

\subsubsection{Recovery}
After you have booted the recovery system, log in with username \user{root} and
type
\begin{commands}{Recovery}
<command></command><parameter>rear recover</parameter>
\end{commands}
...
found under the /mnt/local directory.
