...
\index[general]{Before Running Bareos }
...
Assuming you have built from source or have installed the packages,
...
The rpm installation also creates the group bareos if it does not exist on the
system. Any users that you add to the group bareos will have access to files
...
For simplicity, here we will describe only the {\bf bconsole} program.
...
\TODO{get output from real test environment}
\TODO{auto on? messages?}
...
\item [-c {\textless}file{\textgreater}]
   \index[sd]{-c {\textless}file{\textgreater} }
