...
\index[general]{PostgreSQL Backup}
\index[general]{Database!PostgreSQL!Backup}
\label{back-postgresql}

The RunBeforeJob directive is configured inside a Job Resource, e.g.:

\begin{bconfig}{RunBeforeJob job resource PostgreSQL dump}
Job {
  Name = "BackupDatabase"
  JobDefs = "DefaultJob"
  Level = Full
  ...
  RunBeforeJob = "pg_dumpall -U postgres > postgresql_dump.sql"
...
\begin{bconfig}{RunBeforeJob fileset PostgreSQL dump}
FileSet {
  Name = "Database"
  Include {
    Options {
    ...
    File = "/var/lib/bareos/postgresql_dump.sql" # database dump
  }
}
\end{bconfig}

\subsubsection{Backup of a local PostgreSQL Databases by using the bpipe plugin}
\index[general]{PostgreSQL Backup}
\index[general]{Database!PostgreSQL!Backup}
\label{backup-postgresql}
...
\subsubsection{Backup of a remote PostgreSQL Database by using the bpipe plugin}
\index[general]{PostgreSQL Backup}
\index[general]{Database!PostgreSQL!Backup}
\label{remote-backup-postgresql}

For general information about bpipe, see the \ilink{bpipe plugin}{bpipe} section.
The bpipe plugin is configured inside the Include section of a File Set, e.g.:

\begin{bconfig}{bpipe directive PostgreSQL dump remote host}
FileSet {
  Name = "postgresql-all"
  Include {
    Plugin = "bpipe:/POSTGRESQL/dump.sql:pg_dumpall -h <hostname> -p <port> -U <username> -W <password>:psql -h <hostname> -p <port> -U <username> -W <password>"
    Options {
      signature = MD5
      compression = gzip
    }
...
\index[general]{MySQL Backup}
\index[general]{Database!MySQL!Backup}
\index[general]{RunBeforeJob!MySQL}
\label{backup-mysql}

The RunBeforeJob directive is configured inside a Job Resource, e.g.:

\begin{bconfig}{RunBeforeJob job resource MySQL dump}
#
# Backup our database after the nightly save
#
Job {
  Name = "BackupDatabase"
  JobDefs = "DefaultJob"
...
# This is the backup of the database dump
FileSet {
  Name = "Database"
  Include {
    Options {
  ...
  File = "/var/lib/bareos/mysql_dump.sql" # database dump
  }
}
\end{bconfig}


\subsubsection{Backup a local MySQL Databases by using the bpipe plugin}
\index[general]{MySQL Backup}
\index[general]{Database!MySQL!Backup}
\index[general]{plugin!bpipe!MySQL}
\label{backup-mysql}

For general information about bpipe, see the \ilink{bpipe plugin}{bpipe} section.
The bpipe plugin is configured inside the Include section of a File Set, e.g.:

\begin{bconfig}{bpipe fileset MySQL local dump}
FileSet {
  Name = "mysql-all"
  Include = {
...
\subsubsection{Backup a remote MySQL Database by using the bpipe plugin}
\index[general]{MySQL Backup}
\index[general]{Database!MySQL!Backup}
\index[general]{plugin!bpipe!MySQL}
\label{backup-mysql}
...
\begin{bconfig}{bpipe fileset MySQL remote dump}
FileSet{
  Name = "mysql-all"
  Include {
