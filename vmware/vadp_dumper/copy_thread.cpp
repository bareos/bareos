...
static void *copy_thread(void *data)
{
   ...
   while (1) {
      ...
      while (save_data) {
         cnt = cp_thread->output_function(save_data->sector_offset, save_data->data_len, save_data->data);
         if (cnt < save_data->data_len) {
            return NULL;
         }
         save_data = (CP_THREAD_SAVE_DATA *)context->cb->dequeue();
      }
...
/*
 * Read a new piece of data via the input_function and put it onto the circular buffer.
 */
bool send_to_copy_thread(size_t sector_offset, size_t nbyte)
{
   ...
   save_data->data_len = cp_thread->input_function(sector_offset, nbyte, save_data->data);
   if (save_data->data_len < 0) {
      return false;
   }
    ...
    */
   if (!cp_thread->started) {
      pthread_cond_signal(&cp_thread->start);
   }
