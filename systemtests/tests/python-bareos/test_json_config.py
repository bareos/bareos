...
class PythonBareosJsonConfigTest(bareos_unittest.Json):
    ...
    def test_configure_delete(self):
        ...
        # the refusal names what still references the resource, so that a
        # caller can act on it without having to parse the error text.
        delete_result = context.exception.jsondata["error"]["data"]["result"][
            "configure"
        ]["delete"]
        self.assertEqual(delete_result["name"], referenced_client)
        self.assertEqual(
            delete_result["references"],
            [{"resource": "Job", "name": referencing_job, "directive": "Client"}],
        )
