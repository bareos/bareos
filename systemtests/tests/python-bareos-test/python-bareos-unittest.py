...
class PythonBareosExceptionsTest(PythonBareosBase):
    ...
    def test_exception_connection_error(self):
        ...
        try:
            ...
            logger.debug('signal: str={}, repr={}, args={}'.format(str(e), repr(e), e.args))
    ...
    def _test_job_result(self, jobs, jobid):
        ...
        for job in jobs:
            if job['jobid'] == jobid:
                ...
                self.assertTrue(files >= 1, u'Job {} contains no files ({}).'.format(jobid, files))
                return True
...
class PythonBareosJsonFunctionalTest(PythonBareosJsonBase):
    ...
    def test_json_list_media_with_pool_acl(self):
        ...
        self.assertTrue(int(result['volumes'][0]['count']) >= 1, u'Full pool contains no volumes.')

        result = director_root.call('list volume pool=Incremental count')
        self.assertTrue(int(result['volumes'][0]['count']) >= 1, u'Incremental pool contains no volumes.')
