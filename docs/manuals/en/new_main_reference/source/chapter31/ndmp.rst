...
:index:`NDMP: File information are not available in the Bareos backup stream. <triple: Limitation; NDMP; File information are not available in the Bareos backup stream>`
   As hardlink information is only stored in the Bareos database, but not int the backup stream itself, it is not possible to recover the file history information from the NDMP stream with :program:`bscan`.
