...
mkdir -p ${WORK_DIR}/config-directive-description/
...
if [ $PHASE == "ALL" ]; then
    tExt=".rst.inc"
    #set -x
    for i in build/config-directive-description/*.tex; do
        ...
        if [ $PHASE == "ALL" ] || [ $PHASE == "pandoc" ]; then
            printf "%-50s (%s)\n" "$destfile" "pandoc"
            pandoc --verbose --columns=500  -f latex+raw_tex -t rst ${WORK_DIR}${destfile} -o ${WORK_DIR}${chapterdir}/${filebase}${tExt} || exit "could not convert file ${WORK_DIR}${destfile}"
            #pandoc --verbose --columns=500  -f latex+raw_tex -t rst --filter latex-scan.py ${TARGET_DIR}${destfile} -o ${TARGET_DIR}${chapterdir}/${filebase}.rst || exit "could not convert file $file"
        fi
        ...
        if [ $PHASE == "ALL" ] || [ $PHASE == "postpandoc" ]; then
            ...
            #printf "%s\n"   ".. ATTENTION do not edit this file manually." > ${TARGET_DIR}${chapterdir}/${filebase}.rst
            #printf "%s\n\n" "   It was automatically converted from the corresponding .tex file" >> ${TARGET_DIR}${chapterdir}/${filebase}.rst
            cat ${WORK_DIR}${chapterdir}/${filebase}${tExt} | ./latex-scan.py --standalone > ${TARGET_DIR}${chapterdir}/${filebase}${tExt} 2>${WORK_DIR}${chapterdir}/${filebase}.latex-scan.log || exit "failed to post convert ${WORK_DIR}${chapterdir}/${filebase}${tExt}"
        fi
    done
fi
