...
The dedupable storage backend is a way for bareos to store its data in
...
This means that to support dedupable filesystems we have to
essentially parse the interleaved data stream out into separate data
& metadata streams again and store them on disk in a way where we
...
The basic algorithm to parse this stream is then simply:
...
Now that we have seperated out all block headers, record headers
