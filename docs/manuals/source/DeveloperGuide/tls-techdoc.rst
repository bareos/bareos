...
authentication. If the identity cannot be mapped to a resource,
``BareosAccept()`` rejects the connection instead of dereferencing a
null pointer.
...
extra fields (currently ``fd_protocol_version`` for File daemon
connections). Pre-18.2 console hello messages are rejected by
``parse_hello()``.
