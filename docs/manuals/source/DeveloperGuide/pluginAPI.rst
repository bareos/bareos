...
There is currently one sample program **example-plugin-fd.c** and one
working plugin **bpipe-fd.c** that can be found in the Bareos
**src/plugins/fd** directory. Both are built with the following:
...
the **make** command will build the **bpipe-fd.so** plugin, which is a
very useful and working program.

The **make test** command will build the **example-plugin-fd.so** plugin
and a binary named **main**, which is build from the source code located
in **src/filed/fd_plugins.c**.

If you execute **./main**, it will load and run the example-plugin-fd
plugin simulating a small number of the calling sequences that Bareos
uses in calling a real plugin. This allows you to do initial testing of
your plugin prior to trying it with Bareos.
