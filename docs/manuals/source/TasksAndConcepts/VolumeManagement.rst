...
Above, we discussed how you could have a single device named :config:option:`Sd/Device = FileBackup`\
...
be re-read on any compatible tape drive. The compatibility being determined by the Media Type (:config:option:`dir/storage/MediaType`\
and :config:option:`sd/device/MediaType`\ ). The same applies to disk storage. Since a volume that is
written by a Device in say directory :file:`/var/lib/bareos/storage/backups` cannot be read by a Device
with an :config:option:`sd/device/ArchiveDevice`\  = :file:`/var/lib/bareos/storage/client1`, you will
...
The following example is not very practical, but can be used to demonstrate the proof of concept in
...
.. literalinclude:: ../../../../core/src/stored/backends/unix_file_device.d/bareos-dir.d/storage/Filechanger.conf.example.in
