 ...
 */
static inline void fill_runtime_disk_info_encoding(struct runtime_disk_info_encoding *rdie)
{
...
/*
 * Dump the important content of the internal disk representation for verbose mode.
 */
static inline void dump_runtime_disk_info_encoding(struct runtime_disk_info_encoding *rdie)
{
...
/*
 * Validate the disk sizes from the internal disk representation to the current VDMK settings.
 */
static inline char validate_runtime_disk_info_encoding(struct runtime_disk_info_encoding *rdie)
{
   if (info->biosGeo.cylinders > 0 && info->biosGeo.cylinders < rdie->bios_cylinders) {
      fprintf(stderr, "[validate_runtime_disk_info_encoding] New disk has %ld BIOS cylinders original had %ld\n",
              info->biosGeo.cylinders, rdie->bios_cylinders);
      goto bail_out;
   }

   if (info->biosGeo.heads > 0 && info->biosGeo.heads < rdie->bios_heads) {
      fprintf(stderr, "[validate_runtime_disk_info_encoding] New disk has %ld BIOS heads original had %ld\n",
              info->biosGeo.heads, rdie->bios_heads);
      goto bail_out;
   }

   if (info->biosGeo.cylinders > 0 && info->biosGeo.cylinders < rdie->bios_cylinders) {
      fprintf(stderr, "[validate_runtime_disk_info_encoding] New disk has %ld BIOS sectors original had %ld\n",
              info->biosGeo.sectors, rdie->bios_sectors);
      goto bail_out;
   }

   if (info->physGeo.cylinders < rdie->phys_cylinders) {
      fprintf(stderr, "[validate_runtime_disk_info_encoding] New disk has %ld PHYS cylinders original had %ld\n",
              info->physGeo.cylinders, rdie->phys_cylinders);
      goto bail_out;
   }

   if (info->physGeo.heads < rdie->phys_heads) {
      fprintf(stderr, "[validate_runtime_disk_info_encoding] New disk has %ld PHYS heads original had %ld\n",
              info->biosGeo.heads, rdie->phys_heads);
      goto bail_out;
   }

   if (info->physGeo.cylinders < rdie->phys_cylinders) {
      fprintf(stderr, "[validate_runtime_disk_info_encoding] New disk has %ld PHYS sectors original had %ld\n",
 ...
 */
static inline void do_vixdisklib_connect(const char *key, json_t *connect_params,
                                         bool readonly, bool need_snapshot_moref)
{
   int succeeded = 0;
   VixError err;
   ...
   err = VixDiskLib_InitEx(VIXDISKLIB_VERSION_MAJOR,
...
static inline bool save_disk_info(const char *key, json_t *cbt, uint64_t *absolute_disk_length)
{
   ...
   if (robust_writer(STDOUT_FILENO, (char *)&rdie, rdie_size) != rdie_size) {
      fprintf(stderr, "Failed to write runtime_disk_info_encoding structure to output datastream\n");
      goto bail_out;
   }
...
static inline bool process_disk_info(bool validate_only, json_t *value)
{
   ...
   if (rdie.start_magic != BAREOSMAGIC) {
      fprintf(stderr, "[runtime_disk_info_encoding] Failed to find valid MAGIC start marker read %lld should have been %lld\n", rdie.start_magic, BAREOSMAGIC);
      goto bail_out;
   }

   if (rdie.end_magic != BAREOSMAGIC) {
      fprintf(stderr, "[runtime_disk_info_encoding] Failed to find valid MAGIC end marker read %lld should have been %lld\n", rdie.end_magic, BAREOSMAGIC);
      goto bail_out;
   }
   ...
   if (!validate_only && check_size) {
      if (!validate_runtime_disk_info_encoding(&rdie)) {
         fprintf(stderr, "[runtime_disk_info_encoding] Invalid disk geometry for restoring to this volume\n");
         goto bail_out;
      }
   }
...
static inline bool read_meta_data_key(char *key)
{
   ...
   if (!buffer) {
      fprintf(stderr, "Failed to allocate memory for holding metadata keys, exiting ...\n");
      return false;
   }
   ...
   if (robust_writer(STDOUT_FILENO, &rmde, rmde_size) != rmde_size) {
      fprintf(stderr, "Failed to write runtime_meta_data_encoding structure to output datastream\n");
      goto bail_out;
   }

   if (robust_writer(STDOUT_FILENO, key, rmde.meta_key_length) != rmde.meta_key_length) {
      fprintf(stderr, "Failed to write meta data key to output datastream\n");
      goto bail_out;
   }

   if (robust_writer(STDOUT_FILENO, buffer, rmde.meta_data_length) != rmde.meta_data_length) {
...
static inline bool process_meta_data(bool validate_only)
{
   ...
   while (1) {
      if (robust_reader(STDIN_FILENO, &rmde, rmde_size) != rmde_size) {
         fprintf(stderr, "Failed to read runtime_meta_data_encoding structure from input datastream\n");
         goto bail_out;
      }

      if (rmde.start_magic != BAREOSMAGIC) {
         fprintf(stderr, "[runtime_meta_data_encoding] Failed to find valid MAGIC start marker read %lld should have been %lld\n", rmde.start_magic, BAREOSMAGIC);
         goto bail_out;
      }

      if (rmde.end_magic != BAREOSMAGIC) {
         fprintf(stderr, "[runtime_meta_data_encoding] Failed to find valid MAGIC end marker read %lld should have been %lld\n", rmde.end_magic, BAREOSMAGIC);
         goto bail_out;
      }
      ...
      if (robust_reader(STDIN_FILENO, key, rmde.meta_key_length) != rmde.meta_key_length) {
      ...
      if (robust_reader(STDIN_FILENO, buffer, rmde.meta_data_length) != rmde.meta_data_length) {
 ...
 * to the output stream. We self encode the data using a prefix header that describes
 * the data that is encoded after it including a MAGIC key and the actual CBT information
 * e.g. start of the read sectors and the number of sectors that follow.
 */
static inline bool process_cbt(const char *key, json_t *cbt)
{
   ...
   /*
    * Iterate over each element of the JSON array and get the "start" and "length" member.
    */
   ...
   json_array_foreach(object, index, array_element) {
      ...
      if (robust_writer(STDOUT_FILENO, (char *)&rce, rce_size) != rce_size) {
         fprintf(stderr, "Failed to write runtime_cbt_encoding structure to output datastream\n");
         goto bail_out;
      }
      ...
      while (current_offset < max_offset) {
          ...
          * of sectors still available in this CBT range or the upper setting specified
          * in the sectors_per_call variable.
          */
         sectors_to_read = MIN(sectors_per_call, (offset_length / DEFAULT_SECTOR_SIZE));

         if (multi_threaded) {
            if (!send_to_copy_thread(sector_offset, sectors_to_read * DEFAULT_SECTOR_SIZE)) {
               goto bail_out;
            }
         } else {
...
static inline bool process_restore_stream(bool validate_only, json_t *value)
{
    ...
    */
   if (!process_meta_data(validate_only)) {
      goto bail_out;
   }
   ...
   while (robust_reader(STDIN_FILENO, (char *)&rce, rce_size) == rce_size) {
      ...
      if (rce.end_magic != BAREOSMAGIC) {
         fprintf(stderr, "[runtime_cbt_encoding] Failed to find valid MAGIC end marker read %lld should have been %lld\n", rce.end_magic, BAREOSMAGIC);
         goto bail_out;
      }
      ...
      while (current_offset < max_offset) {
          ...
          * in the sectors_per_call variable.
          */
         sectors_to_read = MIN(sectors_per_call, (rce.offset_length / DEFAULT_SECTOR_SIZE));
         ...
         if (!validate_only && multi_threaded) {
            ...
            cnt = robust_reader(STDIN_FILENO, (char *)buf, sectors_to_read * DEFAULT_SECTOR_SIZE);
            if (cnt != sectors_to_read * DEFAULT_SECTOR_SIZE) {
               goto bail_out;
            }

            if (!validate_only) {
               if (write_to_vmdk(sector_offset, cnt, buf) != cnt) {
                  goto bail_out;
               }
            }
         }
...
/*
 * All work to this program is passed in using a JSON work file which has the needed
 * information to perform the wanted operation. This function loads the JSON data into
 * memory using the Jansson JSON library.
 */
static inline void process_json_work_file(const char *json_work_file)
{
   ...
   if (verbose) {
       ...
       */
      json_dumpf(json_config, stderr, JSON_PRESERVE_ORDER | JSON_COMPACT | JSON_INDENT(3));
      fprintf(stderr, "\n");
      fflush(stderr);
   }
}
