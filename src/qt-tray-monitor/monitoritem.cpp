...
bool MonitorItem::doconnect()
{
  ...
  case R_DIRECTOR:
	 ...
	 message =tr("Connecting to Director %1:%2").arg(dird->address).arg(dird->DIRport);
  ...
  case R_CLIENT:
	 ...
	 message = tr("Connecting to Client %1:%2").arg(filed->address).arg(filed->FDport);
	 Q_EMIT showStatusbarMessage(message);
	 d->DSock =  New(BSOCK_TCP);
  ...
  case R_STORAGE:
	 ...
	 message = tr("Connecting to Storage %1:%2").arg(stored->address).arg(stored->SDport);
	 Q_EMIT showStatusbarMessage(message);
	 d->DSock =  New(BSOCK_TCP);
  ...
  if (d->DSock == NULL) {
	 ...
	 Q_EMIT showStatusbarMessage(tr("Cannot connect to daemon."));
	 Q_EMIT clearText(name);
	 Q_EMIT appendText(name, tr("Cannot connect to daemon."));
	 d->state = MonitorItem::Error;
	 Q_EMIT statusChanged(name, d->state);
	 return false;
  }
  ...
  if (!authenticate_daemon(this, &jcr)) {
	 ...
	 d->state = MonitorItem::Error;
	 Q_EMIT statusChanged(name, d->state);
	 message = tr("Authentication error : %1").arg(d->DSock->msg);
	 Q_EMIT showStatusbarMessage(message);
	 Q_EMIT clearText(name);
	 Q_EMIT appendText(name, tr("Authentication error : %1").arg(d->DSock->msg));
  ...
  case R_DIRECTOR:
	 Q_EMIT showStatusbarMessage(tr("Opened connection with Director daemon."));
	 break;

  case R_CLIENT:
	 Q_EMIT showStatusbarMessage(tr("Opened connection with File daemon."));
	 break;

  case R_STORAGE:
	 Q_EMIT showStatusbarMessage(tr("Opened connection with Storage daemon."));
	 break;

  default:
	 Q_EMIT showStatusbarMessage(tr("Error, currentitem is not a Client, a Storage or a Director..\n"));
	 d->state = Error;
	 Q_EMIT statusChanged(name, d->state);
	 return false;
  }
...
bool MonitorItem::docmd(const char* command)
{
   ...
   while (1) {
	  ...
	  if ((stat = bnet_recv(d->DSock)) >= 0) {
		 ...
		 Q_EMIT appendText(QString::fromUtf8(get_name()), msg);
		 if (d->type == R_CLIENT) {
		 ...
		 if (d->DSock->msglen == BNET_EOD) {
			// qDebug() << "<< EOD >>";
			 if (d->type == R_CLIENT)
				Q_EMIT jobIsRunning (jobRunning);
			return true;
		 ...
		 d->DSock = NULL;
		 d->state = MonitorItem::Error;
		 Q_EMIT statusChanged(get_name(), d->state);
		 Q_EMIT showStatusbarMessage(tr("Error : BNET_HARDEOF or BNET_ERROR"));
	  ...
	  if (is_bnet_stop(d->DSock)) {
		 ...
		 d->state = MonitorItem::Error;
		 Q_EMIT statusChanged(get_name(), d->state);
		 Q_EMIT showStatusbarMessage(tr("Error : Connection closed."));
		 //fprintf(stderr, "<< STOP >>\n");
