...
*/
/*
 * Bacula sql pooling code that manages the database connection pools.
 *
   ...
   foreach_dlist(spd, db_pooling_descriptors) {
      ...
      foreach_dlist(spe, spd->pool_entries) {
         if (spe->reference_count == 0) {
            Dmsg3(100, "db_sql_pool_flush destroy db pool connection %d to %s, backend type %s\n",
                  spe->id, spe->db_handle->get_db_name(), spe->db_handle->db_get_type());
            db_close_database(NULL, spe->db_handle);
            free(spe);
            spd->nr_connections--;
      ...
      /*
       * See if there is anything left on this pool.
       */
      if (spd->nr_connections == 0) {
