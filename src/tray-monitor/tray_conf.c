...
static RES_ITEM mon_items[] = {
   ...
   { "fdconnecttimeout", store_time,ITEM(res_monitor.FDConnectTimeout), 0, ITEM_DEFAULT, "180" /* 60 * 30 */ },
   { "sdconnecttimeout", store_time,ITEM(res_monitor.SDConnectTimeout), 0, ITEM_DEFAULT, "180" /* 60 * 30 */ },
      ...
      if (!res_head[rindex]) {
        res_head[rindex] = (RES *)res; /* store first entry */
         ...
         for (last=next=res_head[rindex]; next; next=next->next) {
            last = next;
            if (bstrcmp(next->name, res->res_monitor.hdr.name)) {
