...
bool BRSRES::print_config(POOL_MEM &buff)
{
   ...
   pm_strcat(cfg_str, my_config->res_to_str(this->hdr.rcode));
   pm_strcat(cfg_str, " {\n");
   ...
   for (i = 0; items[i].name; i++) {
      ...
      /*
       * Skip any items which are aliases for others.
       */
 ...
 */
void dump_resource(CONFIG *config, int type, RES *ures, void sendit(void *sock, const char *fmt, ...), void *sock)
{
   RES *next;
   URES *res = (URES *)ures;
   bool recurse = true;
...
void save_resource(CONFIG *config, int type, RES_ITEM *items, int pass)
{
   ...
   if (pass == 2) {
      ...
      case R_POOL:
          ...
          */
         if ((res = (URES *)config->GetResWithName(R_POOL, res_all.res_con.hdr.name)) == NULL) {
         ...
         if (res_all.res_pool.catalog || !res->res_pool.use_catalog) {
             ...
             */
            res->res_pool.catalog = (CATRES *)config->GetNextRes(R_CATALOG, NULL);
         }
         break;
      case R_CONSOLE:
         if ((res = (URES *)config->GetResWithName(R_CONSOLE, res_all.res_con.hdr.name)) == NULL) {
            Emsg1(M_ERROR_TERM, 0, _("Cannot find Console resource %s\n"), res_all.res_con.hdr.name);
         }
         res->res_con.tls_allowed_cns = res_all.res_con.tls_allowed_cns;
         break;
      case R_DIRECTOR:
         if ((res = (URES *)config->GetResWithName(R_DIRECTOR, res_all.res_dir.hdr.name)) == NULL) {
         ...
         break;
      case R_STORAGE:
         if ((res = (URES *)config->GetResWithName(type, res_all.res_store.hdr.name)) == NULL) {
      ...
      case R_JOBDEFS:
      case R_JOB:
         if ((res = (URES *)config->GetResWithName(type, res_all.res_dir.hdr.name)) == NULL) {
         ...
         break;
      case R_COUNTER:
         if ((res = (URES *)config->GetResWithName(R_COUNTER, res_all.res_counter.hdr.name)) == NULL) {
         ...
         break;
      case R_CLIENT:
         if ((res = (URES *)config->GetResWithName(R_CLIENT, res_all.res_client.hdr.name)) == NULL) {
         ...
         if (res_all.res_client.catalog) {
             ...
             * No catalog overwrite given use the first catalog definition.
             */
            res->res_client.catalog = (CATRES *)config->GetNextRes(R_CATALOG, NULL);
         }
      ...
      case R_SCHEDULE:
          ...
          */
         if ((res = (URES *)config->GetResWithName(R_SCHEDULE, res_all.res_client.hdr.name)) == NULL) {
 ...
 * later from the SD.
 */
static void store_device(CONFIG *config, LEX *lc, RES_ITEM *item, int index, int pass)
{
   int rindex = R_DEVICE - R_FIRST;

   if (pass == 1) {
      lex_get_token(lc, T_NAME);
      config->insert_resource(rindex, resources[rindex].size, lc);
      scan_to_eol(lc);
      set_bit(index, res_all.hdr.item_present);
   } else {
      config->store_resource(CFG_TYPE_ALIST_RES, lc, item, index, pass);
   }
}
 ...
 */
static void store_autopassword(CONFIG *config, LEX *lc, RES_ITEM *item, int index, int pass)
{
   ...
   case R_DIRECTOR:
      ...
      switch (item->code) {
      case 1:
         config->store_resource(CFG_TYPE_CLEARPASSWORD, lc, item, index, pass);
         break;
      default:
         config->store_resource(CFG_TYPE_MD5PASSWORD, lc, item, index, pass);
         break;
      }
      break;
   case R_CLIENT:
 ...
 */
static void store_runscript_target(CONFIG *config, LEX *lc, RES_ITEM *item, int index, int pass)
{
   lex_get_token(lc, T_STRING);
   ...
   if (pass == 2) {
      if (bstrcmp(lc->str, "%c")) {
         ...
         RES *res = config->GetResWithName(R_CLIENT, lc->str);
         if (res == NULL) {
 ...
 */
static void store_runscript(CONFIG *config, LEX *lc, RES_ITEM *item, int index, int pass)
{
   char *c;
   int token, i, t;
   alist **runscripts = (alist **)(item->value) ;
      ...
      for (i = 0; runscript_items[i].name; i++) {
        if (bstrcasecmp(runscript_items[i].name, lc->str)) {
           ...
           case CFG_TYPE_RUNSCRIPT_TARGET:
              store_runscript_target(config, lc, &runscript_items[i], i, pass);
              break;
 ...
 */
static void parse_config_cb(CONFIG *config, LEX *lc, RES_ITEM *item, int index, int pass)
{
   switch (item->type) {
   case CFG_TYPE_AUTOPASSWORD:
      store_autopassword(config, lc, item, index, pass);
      break;
   case CFG_TYPE_ACL:
   ...
   case CFG_TYPE_AUTHTYPE:
      ...
      break;
   case CFG_TYPE_DEVICE:
      store_device(config, lc, item, index, pass);
      break;
   case CFG_TYPE_JOBTYPE:
   ...
   case CFG_TYPE_SHRTRUNSCRIPT:
      ...
      break;
   case CFG_TYPE_RUNSCRIPT:
      store_runscript(config, lc, item, index, pass);
      break;
   case CFG_TYPE_MIGTYPE:
...
bool parse_dir_config(CONFIG *config, const char *configfile, int exit_code)
{
   config->init(configfile, NULL, NULL, init_resource_cb, parse_config_cb,
                exit_code, (void *)&res_all, sizeof(res_all), R_FIRST, R_LAST,
                resources);
