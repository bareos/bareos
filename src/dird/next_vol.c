      ...
      if (ok) {
         ...
         if (bstrcmp(mr->VolStatus, "Append") && has_volume_expired(jcr, mr)) {
            if (retry++ < 200) {            /* sanity check */
               ...
               Jmsg(jcr, M_ERROR, 0, _("We seem to be looping trying to find the next volume. I give up.\n"));
            }
         }
      }
   ...
   if (bstrcmp(mr->VolStatus, "Append") && mr->VolJobs > 0) {
      ...
      if ((mr->MaxVolBytes > 0 && mr->VolBytes >= mr->MaxVolBytes)) {
         ...
         if (mr->VolUseDuration <= (now - mr->FirstWritten)) {
            Jmsg(jcr, M_INFO, 0, _("Max configured use duration=%s sec. exceeded. Marking Volume \"%s\" as Used.\n"),
...
void check_if_volume_valid_or_recyclable(JCR *jcr, MEDIA_DBR *mr, const char **reason)
{
   ...
   *reason = _("but should be Append, Purged or Recycle");
   ...
   /*
    * Check retention period from last written, but recycle to within a minute to try to catch close calls ...
    */
