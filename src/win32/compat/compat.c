...
*/
//                              -*- Mode: C++ -*-
// compat.cpp -- compatibilty layer to make bareos-fd run
...
static int statDir(const char *file, struct stat *sb)
{
   ...
   if (*pdwFileAttributes & FILE_ATTRIBUTE_HIDDEN) {
      sb->st_mode |= S_ISVTX; /* use sticky user bit -> hidden */
   }
   if (*pdwFileAttributes & FILE_ATTRIBUTE_ENCRYPTED) {
      sb->st_mode |= S_ISGID; /* use sticky group bit -> encrypted */
   }
   sb->st_mode |= S_IFDIR;
...
static DWORD fill_attribute(DWORD attr, mode_t mode)
{
   ...
   if (mode & S_ISVTX) {                   // The sticky user bit <=> HIDDEN
      attr |= FILE_ATTRIBUTE_HIDDEN;
   } else {
      attr &= ~FILE_ATTRIBUTE_HIDDEN;
   }
   if (mode & S_ISGID) {                   // The sticky group bit <=> ENCRYPTED
